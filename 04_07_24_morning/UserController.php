                ...
                if ($user != null) {
                    if (($user->password) === md5($request->input('Password'))) {
                        ...
                        $response = [
                            'Success' => true,
                            "data" => $data,
                            'Message' => 'Login Successfully', 'User_Type' => $user->user_type, 'Token' => $token, 'status' => 200
                        ];
                    } else {
            ...
            if (isset($input['DIS_ID']) && isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                if (is_numeric($input['SYM_ID'])) {
                    $fxd_banner = DB::table('symptoms')
                        ->select('SYM_ID AS BANNER_ID', 'DIS_ID', 'SYM_NAME AS BANNER_NAME', 'DESCRIPTION', 'BANNER_URL')
    ...
    function vuallreview(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['DR_ID'])) {
                ...
                foreach ($data1 as $row) {
                    $zeroCount = 0;
                    if ($row->FRIENDLY == 0) $zeroCount++;
                    if ($row->SATISFY == 0) $zeroCount++;
                    if ($row->ECONOMY == 0) $zeroCount++;
                    if ($row->RECOMEND == 0) $zeroCount++;
                    if ($zeroCount != 4) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $data3 = DB::table('dashboard')
                    ...
                    ->select([
                        'dashboard.DASH_ID', 'dashboard.DASH_NAME', 'dashboard.PHOTO_URL', 'dashboard.DASH_TYPE',
                        'master_testdata.TEST_ID', 'master_testdata.TEST_SL', 'master_testdata.TEST_NAME',
                        'master_testdata.TEST_CODE', 'master_testdata.TEST_SAMPLE', 'master_testdata.TEST_CATG',
                        'master_testdata.DEPARTMENT', 'master_testdata.TEST_DESC', 'master_testdata.KNOWN_AS',
                        'master_testdata.FASTING', 'master_testdata.GENDER_TYPE', 'master_testdata.AGE_TYPE',
                        'master_testdata.REPORT_TIME', 'master_testdata.PRESCRIPTION', 'master_testdata.ID_PROOF',
                        'master_testdata.QA1', 'master_testdata.QA2', 'master_testdata.QA3',
                        'master_testdata.QA4', 'master_testdata.QA5', 'master_testdata.QA6'
                    ])
