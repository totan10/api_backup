    ...
    function updt_facility_section(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($req->file('file') !== null) {
                $fileExtension = $req->file('file')->getClientOriginalExtension();
                if ($input['KEY_CODE'] === 'H') {
                    $fileName = $input['DASH_SECTION_ID'] . "H." . $fileExtension;
                } elseif ($input['KEY_CODE'] === 'D') {
                    $fileName = $input['DASH_SECTION_ID'] . "D." . $fileExtension;
                } else {
                    $fileName = $input['DASH_SECTION_ID'] . "B." . $fileExtension;
                }
                $req->file('file')->storeAs('service_facility/section', $fileName);
                $url = asset(Storage::url('app/service_facility/section/' . $fileName));
                ...
                $fields1 = [
                    'DASH_SECTION_NAME' => $input['DASH_SECTION_NAME'],
                    'DASH_SECTION_DESC' => $input['DS_DESCRIPTION'],
                    'STATUS' => $input['DS_STATUS'],
                    'POSITION' => $input['DS_POSITION'],
                ];

                if ($input['KEY_CODE'] === 'H') {
                    $fields['DSH_PHOTO_URL'] = $url;
                    $fields1['PHOTO_URL'] = $url;
                } elseif ($input['KEY_CODE'] === 'D') {
                    $fields['DSM_PHOTO_URL'] = $url;
                    $fields1['PHOTO1_URL'] = $url;
                } else {
                    $fields['DS_BANNER_URL'] = $url;
                    $fields1['BANNER_URL'] = $url;
                }
