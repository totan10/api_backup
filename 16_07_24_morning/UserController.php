...
class UserController extends Controller
{
    ...
    function catgdr(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (isset($input['DIS_ID']) && isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $fxd_banner = DB::table('dis_catg')->select('DIS_ID AS BANNER_ID', 'SPECIALITY AS BANNER_NAME', 'DIS_ID', 'DIS_DESC AS DESCRIPTION', 'BANNER_URL')->get();
    ...
    private function getClinicDrDt($latt, $lont, $drid)
    {
        ...
        foreach ($drcl as $row1) {
            ...
            usort($data, function ($item1, $item2) {
                $dateComparison = $item1['SCH_DT'] <=> $item2['SCH_DT'];
                if ($dateComparison === 0) {
                    return $item1['KM'] <=> $item2['KM'];
                }
        ...
        usort($uniqueDRSCH, function ($a, $b) {
            $dateComparison = $a->AVAILABLE_DT <=> $b->AVAILABLE_DT;
            if ($dateComparison === 0) {
                return $a->KM <=> $b->KM;
            }
            return $dateComparison;
