                    ...
                    foreach ($data1 as $row) {
                        ...
                        if (!isset($groupedData[$row->DR_ID])) {
                            ...
                            for ($i = 0; $i < count($nonNullChkInTimes); $i++) {
                                ...
                                if ($currentDate->lessThanOrEqualTo($checkOutTime)) {
                                    if (!in_array($row->{$CHKINSTATUS[$i]}, ['OUT', 'CANCELLED', 'LEAVE'])) {
                                        ...
                                        $nextIndex = $i + 1;
                                        if ($nextIndex < count($chk) && !empty($row->{$chk[$nextIndex]}) && !in_array($row->{$CHKINSTATUS[$nextIndex]}, ['OUT', 'CANCELLED'])) {
...
$uniqueDoctors = collect($doctorsCollection)->unique('DR_ID')->values()->all(); // Ensure unique and convert back to array

usort($uniqueDoctors, function ($item1, $item2) {
    $order = [
        'IN' => 1,
        'TIMELY' => 2,
        'DELAY' => 3,
    ...
    return ($status1 < $status2) ? -1 : 1;
                ...
                $response = ['Success' => true, 'data' => $uniqueDoctors, 'code' => 200];
            } else {
                $response = ['Success' => false, 'Message' => 'Invalid input parameter', 'code' => 422];
            }
        } else {
