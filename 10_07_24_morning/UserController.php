            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                foreach ($data1 as $row) {
                    if (is_numeric($row->SCH_DAY)) {
                        $date = Carbon::createFromDate(date('Y'), $row->START_MONTH, $row->SCH_DAY)
                            ->addMonths($row->MONTH);
                        $sch_dt = ($date->format('Ymd') === $cdt) ? $date->format('Ymd') : Carbon::now()->format('Ymd');
                    } else {
                    ...
                    if ($allTimesPassed === "true") {
                        ...
                        $drstatus = $row->CHK_IN_STATUS3 ?? $row->CHK_IN_STATUS2 ?? $row->CHK_IN_STATUS1 ?? $row->CHK_IN_STATUS;
                    }
                ...
                if (!empty($ldr['livedoctor'])) {
                    ...
                    $filtered_ldr = array_filter($ldr['livedoctor'], function ($doctor) {
                        return in_array($doctor['DR_STATUS'], ["IN", "TIMELY", "DELAY", "OUT", "CANCELLED", "LEAVE"]);
                    });
