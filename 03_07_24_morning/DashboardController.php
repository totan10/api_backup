    ...
    function hndashboard1(Request $request)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                //SECTION-H #### IPD Section
                $fltr_dash = $dash->filter(function ($item) {
                    return strpos($item->TAG_SECTION, 'AH') !== false;
                    // return $item->DASH_SECTION_ID === 'AG';
                });

                $data1 = $fltr_dash->map(function ($item) {
                    return [
                        "DASH_ID" => $item->DASH_ID,
                        "DIS_ID" => $item->DIS_ID,
                        "DASH_SECTION_ID" => $item->DASH_SECTION_ID,
                        ...
                        "DASH_TYPE" => $item->DASH_TYPE,
                        "DASH_DESCRIPTION" => $item->DASH_DESCRIPTION,
                        "URL_IPD_HG" => $item->URL_IPD_HG,
                        "URL_IPD_HGB" => $item->URL_IPD_HGB,
                        "URL_IPD_HI" => $item->URL_IPD_HG,
                        "URL_IPD_HB" => $item->URL_IPD_HGB,
                        "GR_BANNER_URL" => $item->GR_BANNER_URL,
                        "TAG_SECTION" => $item->TAG_SECTION,
                        "DESCRIPTION" => $item->DASH_DESCRIPTION_DESC ?? '',
                    ];
                })->values()->all();
                ...
                foreach ($data1 as $row) {
                    if (!isset($groupedData[$row['DASH_TYPE']])) {
                        ...
                        if (in_array('AH', $tags) || in_array('AG', $tags) || in_array('AI', $tags) || in_array('AM', $tags)) {
                            $photoGrUrl = $row['URL_IPD_HG'];
                            $bannerGrUrl = $row['URL_IPD_HGB'];
                        }
                    ...
                    $photoUrl = $row['URL_IPD_HI'];
                    $bannerUrl = $row['URL_IPD_HB'];
                ...
                $fltr_dash = $dash->filter(function ($item) {
                    return strpos($item->TAG_SECTION, 'AG') !== false;
                    // return $item->DASH_SECTION_ID === 'AG';
                });
                ...
                $data1 = $fltr_dash->map(function ($item) {
                    return [
                        ...
                        "DASH_TYPE" => $item->DASH_TYPE,
                        "DASH_DESCRIPTION" => $item->DASH_DESCRIPTION,
                        "URL_24X7_HG" => $item->URL_24X7_HG,
                        "URL_24X7_HGB" => $item->URL_24X7_HGB,
                        "URL_24X7_HI" => $item->URL_24X7_HI,
                        "URL_24X7_HB" => $item->URL_24X7_HB,
                        "TAG_SECTION" => $item->TAG_SECTION,
                ...
                foreach ($data1 as $row) {
                    if (!isset($groupedData[$row['DASH_TYPE']])) {
                        ...
                        if (in_array('AG', $tags) || in_array('AH', $tags) || in_array('AI', $tags) || in_array('AM', $tags)) {
                            $photoGrUrl = $row['URL_24X7_HG'];
                            $bannerGrUrl = $row['URL_24X7_HGB'];
                        }
                    ...
                    $photoUrl = $row['URL_24X7_HI'];
                    $bannerUrl = $row['URL_24X7_HB'];
    ...
    function dashboard(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $fltr_dash = $dash->filter(function ($item) {
                    return strpos($item->TAG_SECTION, 'AM') !== false && $item->INDASH === 'True';
                });
                ...
                $I_DTL = $fltr_dash->map(function ($item) {
                    return [
                        ...
                        "DASH_URL" => $item->DASH_URL,
                        "DASH_BAK" => $item->DASH_BAK,
                        "URL_2NDGN_MI" => $item->URL_2NDGN_MI,
                        "URL_2NDGN_MG" => $item->URL_2NDGN_MG,
                        "URL_2NDGN_MB" => $item->URL_2NDGN_MB,
                        "URL_2NDGN_MGB" => $item->URL_2NDGN_MGB,
                        "PHOTO_URL" => $item->PHOTO_URL,
                        "BANNER_URL" => $item->BANNER_URL,
                        "STATUS" => $item->STATUS,
                        "QA1" => $item->QA1,
                ...
                foreach ($I_DTL as $row2) {
                    ...
                    if (!isset($groupedData[$row2['CATEGORY']]['DASH_TYPE'][$row2['DASH_TYPE']])) {
                        $groupedData[$row2['CATEGORY']]['DASH_TYPE'][$row2['DASH_TYPE']] = [
                            ...
                            "DASH_TYPE" => $row2['DASH_TYPE'],
                            "DESCRIPTION" => $row2['GR_DESC'],
                            "PHOTO_URL" => $row2['URL_2NDGN_MG'],
                            "BANNER_URL" => $row2['URL_2NDGN_MGB'],
                            "FACILITY_DETAILS" => []
                        ];
                    }
                    ...
                    $groupedData[$row2['CATEGORY']]['DASH_TYPE'][$row2['DASH_TYPE']]['FACILITY_DETAILS'][] = [
                        ...
                        "DASH_TYPE" => $row2['DASH_TYPE'],
                        "DESCRIPTION" => $row2['DASH_DESCRIPTION'],
                        "PHOTO_URL" => $row2['URL_2NDGN_MI'],
                        "BANNER_URL" => $row2['URL_2NDGN_MB'],
                        "Questions" => [[
                            "QA1" => $row2['QA1'],
                            "QA2" => $row2['QA2'],
                            "QA3" => $row2['QA3'],
...
// return $I_DTL;
                ...
                // $fltr_dash = $dash->filter(function ($item) {
                //     return strpos($item->TAG_SECTION, 'AH') !== false && $item->INDASH === 'True';
                // });
    ...
    //             $fxd_bnr = DB::table('dc_about')->where(['PHARMA_ID' => $p_id, 'STATUS' => 'Active'])->orderby('POSITION')->get();
    //             $dc_sldr = DB::table('dc_slider')->where(['PHARMA_ID' => $p_id, 'STATUS' => 'Active'])->orderby('POSITION')->get();
    //             $dc_hdr = DB::table('dc_dashboard_header')->get();
    //             $banner = DB::table('banner')->where(['PHARMA_ID' => $p_id, 'STATUS' => 'Active'])->get();
    //             $tot_pkg = DB::table('package')->where('PHARMA_ID', '=', $p_id)->orderby('PKG_SL')->get();
    //             $lab_hdr_dtl = DB::table('l_dashboard_details')
    //                 ->select(
    //                     'ID',
    //                     'SECTION_ID',
    //                     'SECTION_NAME',
    //                     'SECTION_SL',
    //                     'ITEM_NAME as PKG_NAME',
    //                     'ITEM_TYPE',
    //                     'DESCRIPTION',
    //                     'PHOTO_URL as PKG_URL',
    //                     'STATUS',
    //                     'KNOWN_AS',
    //                     'FASTING',
    //                     'GENDER_TYPE',
    ...
    //                     'QA6'
    //                 )
    //                 ->where('STATUS', 'Active')->orderby('SECTION_SL')->get();
    //             //SECTION-A #### SLIDER
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'A';
    //             });
    //             $fltr_sldr = $dc_sldr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'A';
    //             });
    //             $A = array("a" => $fltr_arr->values()->all());
    //             $AD["a_details"] = $fltr_sldr->values()->all();

    //             //SECTION-B #### DASHBOARD
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'B';
    //             });
    //             $B = array("b" => $fltr_arr->values()->all());
    //             $BD["b_details"] = DB::table('dc_dashboard_details')->where('PHARMA_ID', $p_id)->where('STATUS', 'Active')->orderby('POSITION')->get();
    ...
    //                 return $item->SECTION_ID === 'C';
    //             });
    //             $C = array("c" => $fltr_arr->values()->all());
    //             $CD["c_details"] = $fltr_sldr->values()->all();

    //             //SECTION-D #### WHY CHOOSE US
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'D';
    //             });
    //             $fltr_bnr = $fxd_bnr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'D';
    //             });
    //             $D = array("d" => $fltr_arr->values()->all());
    //             $DD["d_details"] = $fltr_bnr->values()->all();

    //             //SECTION-E #### SPECIALIST
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'E';
    //             });
    //             $E = array("e" => $fltr_arr->values()->all());
    //             $E_DTL = DB::table('pharmacy')
    //                 ->join('dr_availablity', 'pharmacy.PHARMA_ID', '=', 'dr_availablity.PHARMA_ID')
    //                 ->join('drprofile', 'dr_availablity.DR_ID', '=', 'drprofile.DR_ID')
    ...
    //                     'disease_catg.DIS_ID',
    //                     'drprofile.D_CATG',
    //                     'disease_catg.DESCRIPTION',
    //                     'disease_catg.PHOTO_URL as SPECIALIST_PHOTO',
    //                     DB::raw('count(*) as TOTAL')
    //                 )
    //                 ->where('pharmacy.PHARMA_ID', '=', $p_id)
    //                 ->groupBy('disease_catg.DIS_ID', 'drprofile.D_CATG', 'disease_catg.PHOTO_URL', 'disease_catg.DESCRIPTION', 'disease_catg.POSITION')
    //                 ->orderby('disease_catg.POSITION')
    //                 ->get()->toArray();
    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });
    //             $E_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $ED["e_details"] = array_values($E_DTL + $E_bnr);

    //             //SECTION-F #### SINGLE TEST
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'F';
    //             });
    //             $F = array("f" => $fltr_arr->values()->all());
    //             $F_DTL = DB::table('clinic_testdata')
    //                 ->select(
    //                     'TEST_ID',
    //                     // 'TEST_SL',
    //                     'TEST_NAME',
    ...
    //             });
    //             $F_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $FD["f_details"] = array_values($F_DTL + $F_bnr);

    //             //SECTION-G #### SYMPTOMS
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'G';
    //             });
    //             $G = array("g" => $fltr_arr->values()->all());
    //             $G_DTL = DB::table('pharmacy')
    //                 ->join('dr_availablity', 'pharmacy.PHARMA_ID', '=', 'dr_availablity.PHARMA_ID')
    //                 ->join('drprofile', 'dr_availablity.DR_ID', '=', 'drprofile.DR_ID')
    //                 ->join('symptom', 'drprofile.DIS_ID', '=', 'symptom.DIS_ID')
    //                 ->distinct('symptom.ITEM_ID')
    //                 ->select(
    //                     'symptom.ITEM_ID AS SYMPTOM_ID',
    //                     'symptom.DIS_ID',
    //                     'symptom.ITEM_NAME AS SYMPTOM',
    //                     'symptom.DIS_CATG',
    //                     'symptom.DASH_PHOTO AS SYMPTOM_DPHOTO',
    ...
    //                 )
    //                 ->where('pharmacy.PHARMA_ID', '=', $p_id)
    //                 ->orderby('symptom.POSITION')
    //                 ->take(10)->get()->toArray();
    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });
    //             $G_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $GD["g_details"] = array_values($G_DTL + $G_bnr);

    //             //SECTION-H #### PROFILE TEST
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'H';
    //             });
    //             $H = array("h" => $fltr_arr->values()->all());
    //             $fltr_hdr = $lab_hdr_dtl->filter(function ($item) {
    //                 return $item->SECTION_ID === 'F';
    //             });
    //             $H_DTL = $fltr_hdr->values()->all();
    ...
    //             $H_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $HD["h_details"] = array_values($H_DTL + $H_bnr);

    //             //SECTION-I #### What makes us special?
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'I';
    //             });
    //             $fltr_bnr = $fxd_bnr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'I';
    //             });
    //             $I = array("i" => $fltr_arr->values()->all());
    //             $I_DTL = $fltr_bnr->values()->all();
    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });
    //             $I_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $ID["i_details"] = array_values($I_DTL + $I_bnr);

    //             //SECTION-J #### TEST BY SYMPTOMS
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'J';
    //             });
    //             $J = array("j" =>  $fltr_arr->values()->all());
    //             $data1 = DB::table('l_dashboard_details')
    //                 ->join('package', 'package.LAB_PKG_ID', '=', 'l_dashboard_details.ID')
    //                 ->join('package_details', 'package.PKG_ID', '=', 'package_details.PKG_ID')
    //                 ->join('clinic_testdata', 'package_details.TEST_ID', '=', 'clinic_testdata.TEST_UC')
    ...
    //                     'clinic_testdata.*',
    //                 )
    //                 ->where(['l_dashboard_details.SECTION_ID' => 'D', 'clinic_testdata.PHARMA_ID' => $p_id])
    //                 ->orderby('l_dashboard_details.SECTION_SL')
    //                 ->get();
    //             $J_DTL = [];
    //             $collection = collect($data1);
    ...
    //             $J_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $JD["j_details"] = array_values($J_DTL + $J_bnr);

    //             //SECTION-K #### Home Collection Banner
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'K';
    //             });
    //             $fltr_sldr = $dc_sldr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'K';
    //             });
    //             $K = array("k" => $fltr_arr->values()->all());
    //             $KD["k_details"] = $fltr_sldr->values()->all();

    //             //SECTION-L #### FAMILY CARE PACKAGE
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'L';
    //             });
    //             $L = array("l" => $fltr_arr->values()->all());
    //             $fltr_hdr = $lab_hdr_dtl->filter(function ($item) {
    //                 return $item->SECTION_ID === 'K';
    //             });
    //             $L_DTL = $fltr_hdr->values()->all();

    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });

    //             $L_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $LD["l_details"] = array_values($L_DTL + $L_bnr);

    //             //SECTION-M #### HEALTH CHECKUP PACKAGE
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'M';
    //             });
    //             $M = array("m" => $fltr_arr->values()->all());
    //             $fltr_hdr = $lab_hdr_dtl->filter(function ($item) {
    //                 return $item->SECTION_ID === 'G';
    //             });
    //             $M_DTL = $fltr_hdr->values()->all();

    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });

    //             $M_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $MD["m_details"] = array_values($M_DTL + $M_bnr);

    //             //SECTION-N #### Child Vacination
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'N';
    //             });
    //             $fltr_sldr = $dc_sldr->filter(function ($item) {
    ...
    //             $N = array("n" => $fltr_arr->values()->all());
    //             $ND["n_details"] = $fltr_sldr->values()->all();

    //             //SECTION-O #### WELLNESS PACKAGE
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'O';
    //             });
    //             $O = array("o" => $fltr_arr->values()->all());
    //             $fltr_hdr = $lab_hdr_dtl->filter(function ($item) {
    //                 return $item->SECTION_ID === 'J';
    //             });
    //             $O_DTL = $fltr_hdr->values()->all();
    ...
    //             $O_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $OD["o_details"] = array_values($O_DTL + $O_bnr);

    //             //SECTION-P #### SPECIAL SERVICES
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'P';
    //             });
    //             $fltr_bnr = $fxd_bnr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'P';
    //             });
    //             $P = array("p" => $fltr_arr->values()->all());
    //             $P_DTL = $fltr_bnr->values()->all();
    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });
    //             $P_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $PD["p_details"] = array_values($P_DTL + $P_bnr);

    //             //SECTION-Q #### ADVANCE EQUIPMENTS
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'Q';
    //             });
    //             $fltr_bnr = $fxd_bnr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'Q';
    //             });
    //             $Q = array("q" => $fltr_arr->values()->all());
    //             $QD["q_details"] =  $fltr_bnr->values()->all();

    //             //SECTION-R #### MEET OUR MEDICAL SPECIALIST
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'R';
    //             });
    //             $R = array("r" => $fltr_arr->values()->all());
    //             $R_DTL = DB::table('drprofile')
    ...
    //                     'dr_availablity.DR_FEES',
    //                 )
    //                 ->where(['dr_availablity.PHARMA_ID' => $p_id, 'dr_availablity.DR_BNR' => 'Yes'])
    //                 ->get()->toArray();
    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });
    //             $R_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $RD["r_details"] = array_values($R_DTL + $R_bnr);

    //             //SECTION-S #### PATIENT REVIEW
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'S';
    //             });
    //             $fltr_bnr = $fxd_bnr->filter(function ($item) {
    ...
    //             });
    //             $S = array("s" => $fltr_arr->values()->all());
    //             $SD["s_details"] = $fltr_bnr->values()->all(); //$S_DTL;

    //             // SECTION-T #### TODAY DOCTOR
    //             date_default_timezone_set('Asia/Kolkata');
    //             $currentTime = Carbon::now();
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'T';
    //             });
    //             $T = array("t" => $fltr_arr->values()->all());

    //             $TD["t_details"] = DB::table('drprofile')
    //                 ->join('dr_availablity', 'drprofile.DR_ID', '=', 'dr_availablity.DR_ID')
    ...
    //                     'dr_availablity.CHK_OUT_TIME'
    //                 )
    //                 ->where(['dr_availablity.SCH_DAY' => $day1, 'dr_availablity.PHARMA_ID' => $p_id])
    //                 ->where('WEEK', 'like', '%' . $weekNumber . '%')
    //                 ->whereTime('dr_availablity.CHK_OUT_TIME', '>', $currentTime->format('h:i A'))
    //                 ->get();

    //             //SECTION-U #### OTHER PATHOLOGY CENTRE
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'U';
    //             });
    //             // $fltr_dr = $dc_dr->filter(function ($item) {
    //             //     return $item->DR_BNR === 'Yes';
    //             // });
    //             $U = array("u" => $fltr_arr->values()->all());
    //             $UD["u_details"]  = []; //$fltr_dr->values()->all();

    //             //SECTION-V #### TEST BY ORGAN
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'V';
    //             });
    //             $fltr_pkg = $tot_pkg->filter(function ($item) {
    //                 return $item->DASH_SECTION_ID === 'T';
    //             });
    //             $V = array("v" => $fltr_arr->values()->all());
    //             $VD["v_details"]  = $fltr_pkg->values()->all();

    //             //SECTION-W #### ADD BANNER
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'W';
    //             });
    //             $fltr_sldr = $dc_sldr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'W';
    //             });
    //             $W = array("w" => $fltr_arr->values()->all());
    //             $WD["w_details"]  = $fltr_sldr->values()->all();

    //             //SECTION-X #### ADD BANNER
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'X';
    //             });
    //             $fltr_sldr = $dc_sldr->filter(function ($item) {
    //                 return $item->SECTION_ID === 'X';
    //             });
    //             $X = array("x" => $fltr_arr->values()->all());
    //             $XD["x_details"]  = $fltr_sldr->values()->all();

    //             //SECTION-Y ### FEMALE PACKAGE
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'Y';
    //             });
    //             $Y = array("y" => $fltr_arr->values()->all());
    //             $fltr_hdr = $lab_hdr_dtl->filter(function ($item) {
    //                 return $item->SECTION_ID === 'P';
    //             });
    //             $Y_DTL = $fltr_hdr->values()->all();

    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });

    //             $Y_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $YD["y_details"] = array_values($Y_DTL + $Y_bnr);

    //             //SECTION-Z #### CHILD PACKAGE
    //             $fltr_arr = $dc_hdr->filter(function ($item) {
    //                 return $item->ID === 'Z';
    //             });
    //             $Z = array("z" => $fltr_arr->values()->all());
    //             $fltr_hdr = $lab_hdr_dtl->filter(function ($item) {
    //                 return $item->SECTION_ID === 'Q';
    //             });
    //             $Z_DTL = $fltr_hdr->values()->all();

    //             $fltr_bnr = $banner->filter(function ($item) {
    //                 return $item->BANNER_TYPE === 'Profile';
    //             });

    //             $Z_bnr["Banner"] = $fltr_bnr->values()->all();
    //             $ZD["z_details"] = array_values($Z_DTL + $Z_bnr);

    //             $data = $A + $AD + $B + $BD + $C + $CD + $D + $DD + $E + $ED + $F + $FD + $G + $GD +
    //                 $H + $HD + $I + $ID + $J + $JD + $K + $KD + $L + $LD + $M + $MD + $N + $ND +
    //                 $O + $OD + $P + $PD + $Q + $QD + $R + $RD + $S + $SD + $T + $TD + $U + $UD + $V + $VD + $W + $WD + $X + $XD + $Y + $YD + $Z + $ZD;

    //             if ($data == null) {
    //                 $response = ['Success' => false, 'Message' => 'Dashboard not prepared.', 'data' => $data];
    //             } else {
    //                 $response = ['Success' => true, 'data' => $data, 'code' => 200];
    //             }
    //         } else {
    ...
    function dashboard1(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $promo_bnr = DB::table('promo_banner')->where('STATUS', 'Active')->get();
                $dash = DB::table('dashboard')->where('CATEGORY', 'like', '%' . 'M' . '%')->orderby('GR_POSITION')->where('STATUS', 'Active')->get();
                $pharma = DB::table('pharmacy')
                    ->leftjoin('dr_availablity', 'pharmacy.PHARMA_ID', '=', 'dr_availablity.PHARMA_ID')
                    // ->leftjoin('clinic_testdata', 'pharmacy.PHARMA_ID', '=', 'clinic_testdata.PHARMA_ID')
                    ->select(
                ...
                //SECTION-#### SPECIALIST
                $SPLST_DTL = DB::table('dis_catg')->select('DIS_ID', 'DASH_ID', 'DIS_TYPE', 'DIS_CATEGORY', 'SPECIALIST', 'SPECIALITY', 'PHOTO_URL')->take(7)->orderBy('DIS_SL')->get()->toArray();
                $fltr_promo_bnr = $promo_bnr->filter(function ($item) {
                    return $item->DASH_SECTION_ID === 'SP';
                });
                ...
                //SECTION-#### SURGERY
                $SURG_DTL = DB::table('surgery')->select('SURG_ID', 'SURG_NAME', 'SURG_TYPE', 'DIS_ID', 'DIS_CATEGORY', 'PHOTO_URL', 'DASH_URL')
                ...
                //SECTION-AJ #### 2nd Opinion
                $fltr_dash = $dash->filter(function ($item) {
                    return $item->DASH_SECTION_ID === 'AJ';
                });
                $AJ_DTL = $fltr_dash->map(function ($item) {
                    return [
                        "DASH_ID" => $item->DASH_ID,
                        "DIS_ID" => $item->DIS_ID,
                        "SYM_ID" => $item->SYM_ID,
