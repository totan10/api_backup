...
class DashboardController extends Controller
{
    ...
    function hndashboard1(Request $request)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $EMG_DTL = DB::table('facility_section')
                    ->join('facility_type', 'facility_section.DASH_SECTION_ID', '=', 'facility_type.DASH_SECTION_ID')
                    ->join('facility', 'facility_type.DASH_TYPE_ID', '=', 'facility.DASH_TYPE_ID')
                    ->where(['facility_section.DS_STATUS' => 'Active', 'facility_type.DT_STATUS' => 'Active', 'facility.DN_STATUS' => 'Active'])
                ...
                $SEV_DTL = DB::table('facility_section')
                    ->join('facility_type', 'facility_section.DASH_SECTION_ID', '=', 'facility_type.DASH_SECTION_ID')
                    ->join('facility', 'facility_type.DASH_TYPE_ID', '=', 'facility.DASH_TYPE_ID')
                    ->where(['facility_section.DS_STATUS' => 'Active', 'facility_type.DT_STATUS' => 'Active', 'facility.DN_STATUS' => 'Active'])
     ...
     * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
      * SIN(RADIANS('$latt'))))),2) as KM")
                ...
                $data = $A + $B + $C + $D + $E + $F + $G + $H + $I + $AJ + $INTL_PAT + $SFH + $AG;
    ...
    function diagdash(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $data = $data->merge($this->getCatgDrDt($latt, $lont));
    ...
    // private function getCatgDrDt($latt, $lont)
    // {
    ...
    //         $nonNullChkInTimes = array_filter([$doctor->CHK_IN_TIME, $doctor->CHK_IN_TIME1, $doctor->CHK_IN_TIME2, $doctor->CHK_IN_TIME3]);

    //         foreach ($nonNullChkInTimes as $i => $chkInTime) {
    //             $checkOutTime = Carbon::parse($doctor->{$chkout[$i]});
    //             if (!empty($doctor->{$delay[$i]})) {
    //                 $checkOutTime->addMinutes($doctor->{$delay[$i]});
    //             }
    ...
    //                         ->where(['APPNT_ID' => $doctor->ID, 'APPNT_DT' => $sch_dt])
    //                         ->count();
    ...
    //         return $statusOrder[$a['DR_STATUS']] <=> $statusOrder[$b['DR_STATUS']] ?: $a['FROM'] <=> $b['FROM'];
    //     });
    //     return $ldr;
    // }
    ...
    private function getCatgDrDt($latt, $lont)
    {
        ...
        foreach ($data1 as $doctor) {
            ...
            // Fetch all appointment data for the doctor and date
        $appointments = DB::table('appointment')
        ->select(
            'APPNT_FROM','APPNT_TO'
        )
        ->where('APPNT_ID', $doctor->ID)
        ->where('APPNT_DT', $sch_dt)
        ->get();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                $promo_bnr1 = DB::table('promo_banner')
                    ->where(['promo_banner.PHARMA_ID' => 0, 'promo_banner.STATUS' => 'Active'])
                    ->select(
                        'promo_banner.*',
                ...
                foreach ($cldata['Clinic_Data'] as $clinic) {
                    if ($clinic->CLINIC_TYPE == 'Hospital') {
                        ...
                        $dashIds = DB::table('hospital_facilities_details')
                            ->select('DASH_ID')
                            ->where('SUPER_SPLTY', 'true')
                            ->distinct()
                            ->pluck('DASH_ID');


                        $SUPS_DTL = (clone $baseQuery)
                            ->whereIn('facility.DASH_ID', $dashIds)
                            ->orderByRaw('CASE WHEN facility_type.DTSL4 IS NULL THEN 1 ELSE 0 END, facility_type.DTSL4 ASC')
                            ->orderByRaw('CASE WHEN facility.DNSL4 IS NULL THEN 1 ELSE 0 END, facility.DNSL4 ASC')
                            ->get();
                        ...
                        $groupedData = [];
                        foreach ($SUPS_DTL as $row2) {
                            $ds_dtl = DB::table('facility_section')->where('DASH_SECTION_ID', 'TV')->first();
                            $DsID = $ds_dtl->DASH_SECTION_ID;
                            $Dsname = $ds_dtl->DASH_SECTION_NAME;
                        ...
                        $SUP["Speciality"] = array_values($groupedData);
...
* SIN(RADIANS('$latt'))))),2) as KM")
                ...
                $TST_DTL = DB::table('clinic_testdata')
                    ...
                    ->select(
                        ...
                        'clinic_testdata.ORGAN_URL',
                        'clinic_testdata.TEST_DESC',
                        'clinic_testdata.DEPARTMENT as CATEGORY',
                        'clinic_testdata.COST',
                        'clinic_testdata.KNOWN_AS',
                        'clinic_testdata.FASTING',
                        'clinic_testdata.GENDER_TYPE',
                ...
                $S_BNR["Symtomatic_Test_Banner"] = DB::table('promo_banner')
                ...
                if (isset($SUP)) {
                    $data = array_merge($data, is_array($SUP) ? $SUP : $SUP->toArray());
                }
