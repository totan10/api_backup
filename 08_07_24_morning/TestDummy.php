...
GROUP BY TEST_ID, clinic_testdata.PHARMA_ID, clinic_testdata.DEPARTMENT, HOME_COLLECT) as clinic_testdata"), function ($join) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            * SIN(RADIANS('$latt'))))),2) as KM"), )
                ...
                $data7 = DB::table('dashboard')
                    ->whereIn('DASH_SECTION_ID', ['B', 'C', 'D', 'G', 'H', 'AJ'])
                    ->where('STATUS', 'Active')
                    ->orderby('DASH_TYPE')
                    ->get();
            ...
            //     if ($row3->DASH_SECTION_ID == 'SR') {
            //         $pkgdtl['Questions'] = [$this->getQuestions($row3, 'DS')];
            //     } else {
            //         $pkgdtl = array_merge($pkgdtl, $this->getQuestions($row3, 'DS'));
            //     }

            //     $this->pushData($data, $row3->ID, $row3->DASH_SECTION_NAME, '', $pkgdtl);
            // }
            ...
            //     if ($row3->DASH_SECTION_ID == 'SR') {
            //         $pkgdtl['Questions'] = [$this->getQuestions($row3, 'DT')];
            //     } else {
            //         $pkgdtl = array_merge($pkgdtl, $this->getQuestions($row3, 'DT'));
            //     }

            //     $this->pushData($data, $row3->DASH_TYPE_ID, $row3->DASH_TYPE, $row3->DASH_SECTION_ID, $pkgdtl);
            // }
            ...
            //     if (strpos($row3->DN_TAG_SECTION, 'SR') !== false) {
            //         $pkgdtl['Questions'] = [$this->getQuestions($row3, 'DN')];
            //     } else {
            //         $pkgdtl = array_merge($pkgdtl, $this->getQuestions($row3, 'DN'));
            //     }

            //     $this->pushData($data, $row3->DASH_ID, $row3->DASH_NAME, $row3->DASH_TYPE_ID, $pkgdtl);
            // }
    ...
    private function pushData(&$data, $id, $name, $type, $details) {
        $data->push([
            "ID" => $id,
            "ITEM_NAME" => $name,
            "FIELD_TYPE" => $type,
    ...
    private function getQuestions($row, $prefix) {
        return [
            "QA1" => $row->{$prefix . 'QA1'},
            "QA2" => $row->{$prefix . 'QA2'},
            "QA3" => $row->{$prefix . 'QA3'},
            ...
            "QA5" => $row->{$prefix . 'QA5'},
            "QA6" => $row->{$prefix . 'QA6'},
            "QA7" => $row->{$prefix . 'QA7'} ?? null,
            "QA8" => $row->{$prefix . 'QA8'} ?? null,
            "QA9" => $row->{$prefix . 'QA9'} ?? null,
        ];
    }
    ...
    function opinion_depts_dummy(Request $req)
    {
        ...
        try {
            ...
            if (!$TD) {
                throw new \Exception('Doctor profile not found or approval status is false.');
            }
            ...
            $response = [
                ...
                'Chember No' => $TD->CHEMBER_NO, // Return the correct chember number
                'CHK_IN_TIME' => $TD->CHK_IN_TIME, // Return the correct CHK_IN_TIME
                'CHK_OUT_TIME' => $TD->CHK_OUT_TIME, // Return the correct CHK_OUT_TIME
                'MAX_BOOK' => $TD->MAX_BOOK, // Return the correct MAX_BOOK
                'code' => 200
            ];
        } catch (\Exception $e) {
            $response = [
