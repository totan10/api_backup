...
class UserController extends Controller
{
    ...
    //             //retrieved all pharmas based on input dis_id
    //             $data = $data->merge($this->getClinic($latt, $lont, $did));
    //             $fxd_banner = DB::table('dis_catg')->select('DIS_ID AS BANNER_ID', 'DIS_ID', 'DIS_DESC AS DESCRIPTION', 'BANNER_URL')->get();
        ...
        if ($request->isMethod('post')) {
            ...
            if (isset($input['DRID']) && isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                if (empty($clinics)) {
                    ...
                    $response = ['Success' => true, 'data' => $clinics, 'code' => 200];
                }
            } else {
                $response = ['Success' => false, 'Message' => 'Invalid input parameter', 'code' => 422];
            }
    ...
    // function nearlivedr(Request $req)
    // {
    //     if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    //         $input = $req->json()->all();
    //         if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
    ...
    //                 ->select('DIS_ID', 'SPECIALIST', 'SPECIALITY', 'PHOTO_URL', 'DIS_CATEGORY')
    //                 ->orderby('DIS_SL')
    //                 ->get();
    ...
    //                 usort($ldr['livedoctor'], function ($item1, $item2) {
    //                     $order = [
    //                         'IN' => 1,
    //                         'TIMELY' => 2,
    //                         'DELAY' => 3,
    ...
    //                 $filtered_ldr = array_filter($ldr['livedoctor'], function ($doctor) {
    ...
    //                 });
    //                 $ldr['livedoctor'] = array_values($filtered_ldr);
    ...
    function nearlivedr(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                //              // Calculate the sum of all non-null max_book values
                //              $totalMaxBook = collect([
                //                 $row->MAX_BOOK,
                //                 $row->MAX_BOOK1,
                //                 $row->MAX_BOOK2,
                //                 $row->MAX_BOOK3
                //             ])->filter()->sum();
                //         if (!is_null($row->$chkInTime)) {
                //             $chkIn = Carbon::parse($row->$chkInTime);
                //             $chkOut = $chkIn->copy()->addMinutes($row->$maxBook * $row->$slotIntvl);

                //             // Add delay if present
                //             if (!is_null($row->$drDelay)) {
                //                 [$delayHours, $delayMinutes] = explode(':', $row->$drDelay);
                ...
                //             if ($currentDate->lessThanOrEqualTo($chkOut)) {
                //                 $doctorStatus = $row->$chkInStatus;
                //                 $chamberNo = $row->$chamberNoField;
                //                 $drArrive = $row->$drArriveField;
                //                 // break;
                //             }
                //             // else{
                //             //     break;
        ...
        * SIN(RADIANS('$latt'))))),2) as KM"),
                        ...
                        DB::raw("
    CASE
       WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME AND dr_availablity.CHK_OUT_TIME THEN dr_availablity.CHK_IN_STATUS
       WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME1 AND dr_availablity.CHK_OUT_TIME1 THEN dr_availablity.CHK_IN_STATUS1
        WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME2 AND dr_availablity.CHK_OUT_TIME2 THEN dr_availablity.CHK_IN_STATUS2
        WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME3 AND dr_availablity.CHK_OUT_TIME3 THEN dr_availablity.CHK_IN_STATUS3
...
//                 ->orderByRaw("
//     CASE 
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME AND dr_availablity.CHK_OUT_TIME THEN FIELD(dr_availablity.CHK_IN_STATUS, 'IN', 'TIMELY', 'DELAY', 'CANCELLED', 'OUT', 'LEAVE')
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME1 AND dr_availablity.CHK_OUT_TIME1 THEN FIELD(dr_availablity.CHK_IN_STATUS1, 'IN', 'TIMELY', 'DELAY', 'CANCELLED', 'OUT', 'LEAVE')
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME2 AND dr_availablity.CHK_OUT_TIME2 THEN FIELD(dr_availablity.CHK_IN_STATUS2, 'IN', 'TIMELY', 'DELAY', 'CANCELLED', 'OUT', 'LEAVE')
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME3 AND dr_availablity.CHK_OUT_TIME3 THEN FIELD(dr_availablity.CHK_IN_STATUS3, 'IN', 'TIMELY', 'DELAY', 'CANCELLED', 'OUT', 'LEAVE')
//         ELSE 6
//     END
// ")
// ->orderByRaw("
//     CASE
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME AND dr_availablity.CHK_OUT_TIME AND dr_availablity.CHK_IN_STATUS = 'IN' THEN 1
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME1 AND dr_availablity.CHK_OUT_TIME1 AND dr_availablity.CHK_IN_STATUS1 = 'IN' THEN 1
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME2 AND dr_availablity.CHK_OUT_TIME2 AND dr_availablity.CHK_IN_STATUS2 = 'IN' THEN 1
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME3 AND dr_availablity.CHK_OUT_TIME3 AND dr_availablity.CHK_IN_STATUS3 = 'IN' THEN 1
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME AND dr_availablity.CHK_OUT_TIME THEN 2
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME1 AND dr_availablity.CHK_OUT_TIME1 THEN 2
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME2 AND dr_availablity.CHK_OUT_TIME2 THEN 2
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME3 AND dr_availablity.CHK_OUT_TIME3 THEN 2
//         ELSE 3
//     END,
//     CASE 
//         WHEN dr_availablity.CHK_IN_STATUS = 'IN' THEN 1
//         WHEN dr_availablity.CHK_IN_STATUS1 = 'IN' THEN 1
//         WHEN dr_availablity.CHK_IN_STATUS2 = 'IN' THEN 1
//         WHEN dr_availablity.CHK_IN_STATUS3 = 'IN' THEN 1
//         WHEN dr_availablity.CHK_IN_STATUS = 'TIMELY' THEN 2
//         WHEN dr_availablity.CHK_IN_STATUS1 = 'TIMELY' THEN 2
//         WHEN dr_availablity.CHK_IN_STATUS2 = 'TIMELY' THEN 2
//         WHEN dr_availablity.CHK_IN_STATUS3 = 'TIMELY' THEN 2
//         ELSE 3
//     END
// ")
...
->orderByRaw("
    CASE
        WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME AND dr_availablity.CHK_OUT_TIME THEN
            CASE WHEN dr_availablity.CHK_IN_STATUS = 'IN' THEN 1
                 WHEN dr_availablity.CHK_IN_STATUS = 'TIMELY' THEN 2
                 ELSE 3
            END
        WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME1 AND dr_availablity.CHK_OUT_TIME1 THEN
            CASE WHEN dr_availablity.CHK_IN_STATUS1 = 'IN' THEN 1
                 WHEN dr_availablity.CHK_IN_STATUS1 = 'TIMELY' THEN 2
                 ELSE 3
            END
        WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME2 AND dr_availablity.CHK_OUT_TIME2 THEN
            CASE WHEN dr_availablity.CHK_IN_STATUS2 = 'IN' THEN 1
                 WHEN dr_availablity.CHK_IN_STATUS2 = 'TIMELY' THEN 2
                 ELSE 3
            END
        WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME3 AND dr_availablity.CHK_OUT_TIME3 THEN
            CASE WHEN dr_availablity.CHK_IN_STATUS3 = 'IN' THEN 1
                 WHEN dr_availablity.CHK_IN_STATUS3 = 'TIMELY' THEN 2
                 ELSE 3
            END
        ELSE 4
...
//     CASE
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME AND dr_availablity.CHK_OUT_TIME THEN
//             CASE dr_availablity.CHK_IN_STATUS
//                 WHEN 'IN' THEN 1
//                 WHEN 'TIMELY' THEN 2
//                 WHEN 'DELAY' THEN 3
...
//             END
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME1 AND dr_availablity.CHK_OUT_TIME1 THEN
//             CASE dr_availablity.CHK_IN_STATUS1
//                 WHEN 'IN' THEN 1
//                 WHEN 'TIMELY' THEN 2
//                 WHEN 'DELAY' THEN 3
//                 WHEN 'CANCELLED' THEN 4
...
//             END
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME2 AND dr_availablity.CHK_OUT_TIME2 THEN
//             CASE dr_availablity.CHK_IN_STATUS2
//                 WHEN 'IN' THEN 1
//                 WHEN 'TIMELY' THEN 2
//                 WHEN 'DELAY' THEN 3
//                 WHEN 'CANCELLED' THEN 4
...
//             END
//         WHEN '$currentTime' BETWEEN dr_availablity.CHK_IN_TIME3 AND dr_availablity.CHK_OUT_TIME3 THEN
//             CASE dr_availablity.CHK_IN_STATUS3
//                 WHEN 'IN' THEN 1
//                 WHEN 'TIMELY' THEN 2
//                 WHEN 'DELAY' THEN 3
//                 WHEN 'CANCELLED' THEN 4
...
//                 ELSE 6
//             END
//         ELSE 7  -- This case is for doctors who do not have the current time within any of their slots
//     END
// ")
                ...
                // Helper function to find the most relevant status
                function getRelevantStatus($doctor, $currentDate)
                {
                    $status = null;
                    for ($i = 0; $i <= 3; $i++) {
                        $chkOutTime = "CHK_OUT_TIME" . ($i == 0 ? "" : $i);
                        ...
                        if (!is_null($doctor->$chkOutTime)) {
                            $chkOut = Carbon::parse(convertToValidTimeFormat($doctor->$chkOutTime));
                            if ($currentDate->lessThanOrEqualTo($chkOut)) {
                                $status = $doctor->$chkInStatus;
                                break;
                            }
                        }
                    }
                ...
                // Helper function to convert time to valid format
                function convertToValidTimeFormat($time)
                {
                    if ($time === '00:00 PM') {
                        return '12:00 PM';
                    }
                ...
                foreach ($data1 as $doctor) {
                    $status = getRelevantStatus($doctor, $currentDate);
                    if ($status) {
                        $sortedDoctors[] = array_merge((array) $doctor, ['current_status' => $status]);
                    }
                }

                // usort($sortedDoctors, function ($item1, $item2) {
                //     $order = [
                //         'IN' => 1,
                //         'TIMELY' => 2,
                //         'DELAY' => 3,
                ...
                //         'LEAVE' => 6
                //     ];
                //     $status1 = $order[$item1['current_status']] ?? 999;
                //     $status2 = $order[$item2['current_status']] ?? 999;
                //     if ($status1 == $status2) {
                //         return 0;
                //     }
                //     return ($status1 < $status2) ? -1 : 1;
                ...
                usort($sortedDoctors, function ($item1, $item2) {
                    ...
                    // Parse the check-in times to Carbon instances for comparison
                    $chkInTime1 = Carbon::parse($item1['CHK_IN_TIME']);
                    $chkInTime2 = Carbon::parse($item2['CHK_IN_TIME']);
                
                    // First, sort by check-in time
                    if ($chkInTime1->eq($chkInTime2)) {
                        // If check-in times are the same, sort by current status
                        $status1 = $order[$item1['current_status']] ?? 999;
                        $status2 = $order[$item2['current_status']] ?? 999;
                        return $status1 <=> $status2;
                    }
                
                    return $chkInTime1 <=> $chkInTime2;
                });
                

                // Output the sorted array of doctors
                ...
                while ($startDate->lte($endDate) && $counter < 6) {
                    ...
                    if ($startDate->format('l') === $row->SCH_DAY) {
                        if (in_array($startDate->weekOfMonth, explode(',', $row->WEEK))) {
                            ...
                            // Calculate available slots and slot status
                            $availableSlots = $totalMaxBook - $totappct;
                            $book_sts = $availableSlots == 0 ? "Closed" : "Available";
                            ...
                            // Calculate TO if it's null
                            $to = $row->CHK_OUT_TIME;
                            $to1 = $row->CHK_OUT_TIME1;
                            $to2 = $row->CHK_OUT_TIME2;
                            $to3 = $row->CHK_OUT_TIME3;

                            if (is_null($to) && !is_null($row->CHK_IN_TIME)) {
                                $to = Carbon::createFromFormat('h:i A', $row->CHK_IN_TIME)
                                    ->addMinutes($row->MAX_BOOK * $row->SLOT_INTVL)
                                    ->format('h:i A');
                            }
                            if (is_null($to1) && !is_null($row->CHK_IN_TIME1)) {
                                $to1 = Carbon::createFromFormat('h:i A', $row->CHK_IN_TIME1)
                                    ->addMinutes($row->MAX_BOOK1 * $row->SLOT_INTVL)
                                    ->format('h:i A');
                            }
                            if (is_null($to2) && !is_null($row->CHK_IN_TIME2)) {
                                $to2 = Carbon::createFromFormat('h:i A', $row->CHK_IN_TIME2)
                                    ->addMinutes($row->MAX_BOOK2 * $row->SLOT_INTVL)
                                    ->format('h:i A');
                            }
                            if (is_null($to3) && !is_null($row->CHK_IN_TIME3)) {
                                $to3 = Carbon::createFromFormat('h:i A', $row->CHK_IN_TIME3)
                                    ->addMinutes($row->MAX_BOOK3 * $row->SLOT_INTVL)
                                    ->format('h:i A');
                            }
            ...
            if (isset($input['FACILITY_ID'])) {
                ...
                foreach ($data1 as $row) {
                    ...
                    $groupedData[$row->DASH_TYPE]['FACILITY_DETAILS'][] = [
                        ...
                        // "PHOTO_URL" => $row->URL_SURGERY_MI,
                        // "BANNER_URL" => $row->DN_BANNER_URL,
    ...
    // function vuhnallsurg(Request $request)
    // {
    //     if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    //         // $headers = apache_request_headers();
    //         // session_start();
    ...
    //                     "DASH_SECTION_ID" => $item->DASH_SECTION_ID,
    //                     "DASH_SECTION_NAME" => $item->DASH_SECTION_NAME,
    //                     "DESCRIPTION" => $item->DASH_SECTION_DESC_SR,
    //                     // "PHOTO_URL" =>  $item->VIEW1_URL,
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $sym = DB::table('symptoms')
                    ...
                    ->select('SYM_ID', 'SYM_NAME', 'DIS_ID', 'DIS_CATEGORY', 'PHOTO_URL', 'SYM_TYPE', 'SYM_SL', 'TYPE_SL')
                    ->where('STATUS', 'Active')
                    ->orderby('SYM_SL')
                    ->orderby('TYPE_SL')
                    ->get();
                $data = array();
                $i = 1;
                $collection = collect($sym);
                $distinctValues = $collection->pluck('SYM_TYPE')->unique();
    ...
    function vuallsplst()
    {
        ...
        $spl = DB::table('dis_catg')
            ->distinct('DIS_TYPE')
            ->select('DIS_ID', 'SPECIALIST', 'DIS_CATEGORY', 'PHOTO_URL', 'DIS_TYPE', 'SPECIALITY', 'TYPE_SL', 'DIS_SL')
            ->distinct('SPECIALIST')
            ->where('STATUS', 'Active')
            ->orderby('TYPE_SL')
            ->orderby('DIS_SL')
    ...
    private function getSpecialist($pharmaId)
    {
        ...
        $data['specialist'] = DB::table('dis_catg')
            ...
            ->select(
                ...
                'dis_catg.DIS_TYPE',
                'dis_catg.PHOTO1_URL AS PHOTO_URL',
                DB::raw('COUNT(DISTINCT dr_availablity.DR_ID) as TOTAL')
            )
            ->where('dr_availablity.PHARMA_ID', '=', $pharmaId)
            ->groupBy(
                'dis_catg.DIS_ID',
    ...
    //         date_default_timezone_set('Asia/Kolkata');
    //         $input = $req->json()->all();
    //         if (isset($input['DR_ID']) && isset($input['PHARMA_ID'])  && isset($input['APPNT_ID'])) {
    //             $drid = $input['DR_ID'];
    //             $fid = $input['PHARMA_ID'];
    //             $apid = $input['APPNT_ID'];
    //             $apdt = $input['APPNT_DT'];
    ...
    //                                     return $item === null ? 0 : (int)$item;
    //                                 })->toArray();
    //                                 $availableAppointments = $row->MAX_BOOK;
    //                             } else {
    //                                 $availableAppointments = $row->MAX_BOOK;
    //                                 $totalAppointments = 0;
    //                                 $BookedSl = [];
    ...
    //                                 $bookingSerials = [];
    //                             } else {
    //                                 $bookingSerials = range($cumulativeAppntCount, $cumulativeAppntCount + $row->MAX_BOOK - 1);
    //                                 $cumulativeAppntCount += $row->MAX_BOOK;
    //                             }
    ...
    //                             // $slotString = $this->createSlot($chkinTime, $chkoutTime, $chkoutTime->diffInMinutes($chkinTime), $row, $apid, $cdt, $apdt);
    //                             // $this->assignSlotToPeriod($slots, $slotString, $chkinTime);
    //                         } else {
    //                             while ($chkinTime->lessThan($chkoutTime)) {
    //                                 $endSlot = $chkinTime->copy()->addMinutes($intvl);
    //                                 if ($endSlot->greaterThan($chkoutTime)) {
...
//                         Log::info('Booked Count: ' . $bookedCount);
    ...
    //                         $bookingSerials = range($cumulativeAppntCount, $cumulativeAppntCount + $maxbk - 1);
//                         $cumulativeAppntCount += $maxbk;

    //                         $availableSerials = [];

    //                         if ($bookedCount > 0) {
//                             $bookedSerials = range(1, $bookedCount);
...
//                             "TO" => $endSlot->format('h:i A'),
//                             "TOTAL_APNT" => $maxbk,
//                             "AVAIL_APNT" => count($availableSerials),
//                             "BOOKING_SERIALS" => $bookingSerials,
...
//                         Log::info('Booked Count: ' . $bookedCount);
    ...
    //                         $bookingSerials = range($cumulativeAppntCount, $cumulativeAppntCount);
//                         $cumulativeAppntCount += 1;
...
// }
                ...
                foreach ($chkInTimes as $index => $chkin) {
                    ...
                    if ($data1->SLOT == '1') {
                        ...
                        while ($chkinTime->lessThan($chkoutTime)) {
                            ...
                            $availableSerials = $totalAppointments - $bookedCount;
                            ...
                            $slotString = [
                                ...
                                "TO" => $endSlot->format('h:i A'),
                                "TOTAL_APNT" => $totalAppointments,
                                "AVAIL_APNT" => $availableSerials,
                                "BOOKING_SERIALS" => $bookingSerials,
                                "AVAILABLE_SERIALS" => array_diff($bookingSerials, range(1, $bookedCount)),
                                "SLOT_STATUS" => $slot_sts,
                            ];
    ...
    //             ->join('facility_section','dashboard.FACILITY_ID','=','facility_section.DASH_SECTION_ID')
    //             ->join('facility_type','facility_section.DASH_SECTION_ID','=','facility_type.DASH_SECTION_ID')
    //             ->join('facility','facility_type.DASH_TYPE_ID','=','facility.DASH_TYPE_ID')

    //             ->where('facility.TAG_SECTION', 'like', '%' . $f_id . '%')
    //                 ->where('STATUS', 'Active')
    //                 ->orderby('GR_POSITION')
    //                 ->get();
    ...
    //         //     $currentTime = Carbon::createFromFormat('h:i A', Carbon::now()->format('h:i A'));
    //         //     $firstRowTOTime = $data[0]['TO'];
    //         //     $firstRowTOTime1 = $data[0]['TO1'];
    //         //     $firstRowTOTime2 = $data[0]['TO2'];
    //         //     $firstRowTOTime3 = $data[0]['TO3'];

    //         //     if ($currentTime->greaterThan($firstRowTOTime) && $currentTime->greaterThan($firstRowTOTime1) && $currentTime->greaterThan($firstRowTOTime2) && $currentTime->greaterThan($firstRowTOTime3)) {
    //         //         $data[0]['DR_STATUS'] = "OUT";
    //         //         $data[0]['SLOT_STATUS'] = "Closed";
    //         //     }
    //         // }
    ...
    // private function getDtSlot($fid, $drid, $apid, $dates)
    // {
    //     $cdt = date('Ymd');
    //     $data1 = DB::table('dr_availablity')
    //         ->where(['DR_ID' => $drid, 'PHARMA_ID' => $fid, 'ID' => $apid])
    ...
    private function getClinic($latt, $lont, $did)
    {
        ...
        //     $clinic->Doctor = $this->getCatgDrDt1($latt, $lont, $did, $clinic->PHARMA_ID);
        //     $data['Clinic'][] = $clinic;
        // }
    ...
    private function getCatgDrDt1($latt, $lont, $did, $pid)
    {
        $totdr = DB::table('drprofile')
            ->join('dr_availablity', 'drprofile.DR_ID', '=', 'dr_availablity.DR_ID')
            ->join('pharmacy', 'dr_availablity.PHARMA_ID', '=', 'pharmacy.PHARMA_ID')
        ...
        foreach ($totdr as $row1) {
            $dravail = DB::table('dr_availablity')
                ...
                ->orderByRaw("FIELD(dr_availablity.CHK_IN_STATUS,'IN','TIMELY','DELAY','CANCELLED','OUT','LEAVE')")
                ->orderby('dr_availablity.CHK_OUT_TIME')->get();
            $totapp = DB::table('appointment')->where(['DR_ID' => $row1->DR_ID, 'PHARMA_ID' => $pid])->get();
            $data = [];
        ...
        $DRSCH = array_values($DRSCH['Doctors']);
        return $DRSCH;
    }
    ...
    private function getClinicDrDt($latt, $lont, $drid)
    {
        ...
        $uniqueDRSCH = array_map('json_decode', $uniqueDRSCH);
