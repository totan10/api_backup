...
class LabController extends Controller
{
    ...
    //                     ->where(['DASH_SECTION_ID' => $row->DASH_SECTION_ID, 'PHARMA_ID' => $firstPharma->PHARMA_ID])
    //                     ->get();
    //             }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['LAB_PKG_ID'])) {
                ...
                $data1 = DB::table('package')
                    ->join('package_details', 'package.PKG_ID', '=', 'package_details.PKG_ID')
                    ->join('pharmacy', 'package.PHARMA_ID', '=', 'pharmacy.PHARMA_ID')
                    ->join('test_schedule', function ($join) {
                        $join->on('package.PHARMA_ID', '=', 'test_schedule.PHARMA_ID')
                            ->on('package.DEPT_ID', '=', 'test_schedule.DEPT_ID')
                            ->where('test_schedule.DEPT_ID', '=', 'D3');
                    })
                    ->distinct('pharmacy.PHARMA_ID')
                    ->select(
                        // 'package.*',
                        // 'package_details.*',
                        // 'pharmacy.*',
                         'package.LAB_PKG_ID', 'package.LAB_PKG_NAME','package.PKG_ID', 'package.PKG_NAME', 'package.PKG_TYPE', 'package.PKG_DESC','package.DEPT_ID', 'package.PKG_URL', 'package.KNOWN_AS',
                    'package.FASTING', 'package.GENDER_TYPE', 'package.AGE_TYPE','package.DASH_SECTION_ID', 'package.REPORT_TIME', 'package.PRESCRIPTION',
                    'package.ID_PROOF', 'package.QA1', 'package.QA2', 'package.QA3', 'package.QA4', 'package.QA5', 'package.QA6',
                    'package.PKG_COST', 'package.PKG_DIS', 'package.HOME_COLLECT', 'package.FREE_AREA', 'package.SERV_CONDITION',
                    'package_details.TEST_ID', 'package_details.TEST_UC', 'package_details.TEST_NAME', 'package_details.TEST_TYPE', 
                    'package_details.COST', 'pharmacy.PHARMA_ID', 'pharmacy.ITEM_NAME', 'pharmacy.ADDRESS', 'pharmacy.CITY', 
                    'pharmacy.DIST', 'pharmacy.CLINIC_MOBILE','pharmacy.CLINIC_TYPE', 'pharmacy.PIN','pharmacy.HO_ID', 'pharmacy.EMAIL', 'pharmacy.STATE', 'pharmacy.LATITUDE',
                    'pharmacy.LONGITUDE', 'pharmacy.PHOTO_URL', 'pharmacy.LOGO_URL', 'pharmacy.PH_RATING',
                        DB::raw("round(111.111 * DEGREES(ACOS(LEAST(1.0, COS(RADIANS(pharmacy.Latitude)) 
                * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
                * SIN(RADIANS('$latt'))))),2) as KM"),
                    )
                    ->where(['package.LAB_PKG_ID' => $L_PKG_ID,'pharmacy.STATUS'=>'Active'])
                    ->orderby('package_details.TEST_ID')
                    ->distinct()
                    ->get();
                ...
                $data2 = DB::table('dashboard_item')
                    ->where('DASH_ID', '=', $L_PKG_ID)
                    ->get();

                $bnr["Catg_Banner"] = $data2->map(function ($item) {
                    // Mapping all BANNER_URL fields
                    $bannerUrls = [];
                    for ($i = 1; $i <= 10; $i++) {
                        $bannerUrls["BANNER_URL$i"] = $item->{"DI_BNR$i"};
                    }

                    // Mapping all QA fields into Questions key
                    $questions = [];
                    for ($i = 1; $i <= 9; $i++) {
                        $questions["QA$i"] = $item->{"DIQA$i"};
                    }

                    return array_merge(
                        [
                            "BANNER_ID" => $item->DASH_ID,
                            "BANNER_NAME" => $item->DASH_NAME,
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['ITEM_NAME'])) {
                ...
                $data = DB::table('clinic_testdata')
                    ...
                    * SIN(RADIANS('$latt'))))),2) as KM"),
                    ...
                    ->where(['clinic_testdata.TEST_CATG' => $test_catg,'pharmacy.STATUS'=>'Active'])
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['TEST_ID'])) {
                ...
                $data1 = DB::table('clinic_testdata')
                    ...
                    * SIN(RADIANS('$latt'))))),2) as KM"),
                    ...
                    ->where(['clinic_testdata.TEST_ID' => $test_id,'pharmacy.STATUS'=>'Active'])
                    ->orderby('clinic_testdata.TEST_NAME')
                    ->get();
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['LAB_PKG_ID']) && isset($input['PHARMA_ID'])) {
                ...
                $data1 = DB::table('package')
                    ...
                    * SIN(RADIANS('$latt'))))),2) as KM"),
                    ...
                    ->where(['package.LAB_PKG_ID' => $L_PKG_ID,'pharmacy.STATUS'=>'Active', 'pharmacy.PHARMA_ID' => $P_ID])
                    ->orderby('package_details.TEST_ID')
                    ->distinct()
                    ->get();
                ...
                // $bnr["Catg_Banner"] = DB::table('dashboard_item')
                //     ->select('DASH_ID AS BANNER_ID', 'DASH_NAME AS BANNER_NAME', 'DASH_DESC AS DESCRIPTION', 'DI_BNR1 AS BANNER_URL')
                //     ->where(['DASH_ID' => $L_PKG_ID])
                //     ->get();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                * SIN(RADIANS('$latt'))))),2) as KM"), )
                    ...
                    // * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
                    //  * SIN(RADIANS('$latt'))))),2) as KM" <= 100)
                    ->where(['PHARMA_ID' => $P_ID,'pharmacy.STATUS'=>'Active'])
                    ->take(25)
                    ->get();
                foreach ($data2 as $row2) {
                    $cldtl = [];
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                $data1 = DB::table('pharmacy')
                    ...
                    ->where(['dr_availablity.SCH_DAY' => $day1,'pharmacy.STATUS'=>'Active'])
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['FAMILY_ID']) && isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $data1 = DB::table('booktest')
                    ...
                    ->where(['user_family.FAMILY_ID'=> $f_id,'pharmacy.STATUS'=>'Active'])
    ...
    //                     "FACILITY_DETAILS" => $T_DTL
    //                 ];
    //             }

    //             $fltr_promo_bnr = $promo_bnr->filter(function ($item) use ($f_id) {
    //                 return $item->DASH_SECTION_ID === 'AM';
    //             });
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['FACILITY_ID'])) {
                ...
                foreach ($distinctValues as $row) {
                    ...
                    $S_DTL['Departments'][] = [
                        "DASH_SECTION_ID" => 'AM',
                        "TAG_SECTION" => $fltr_arr->first()->DT_TAG_SECTION,
                        "DASH_TYPE" => $row,
                        "DESCRIPTION" => $fltr_arr->first()->DT_DESCRIPTION,
                        // "BANNER_URL" => $fltr_arr->first()->PHOTO_URL,
                        "PHOTO_URL1" => $fltr_arr->first()->DTIMG1,

                        "PHOTO_URL2" => $fltr_arr->first()->DTIMG2,
                        "PHOTO_URL3" => $fltr_arr->first()->DTIMG3,
                        "PHOTO_URL4" => $fltr_arr->first()->DTIMG4,
                        "PHOTO_URL5" => $fltr_arr->first()->DTIMG5,
                        "PHOTO_URL6" => $fltr_arr->first()->DTIMG6,
                        "PHOTO_URL7" => $fltr_arr->first()->DTIMG7,
                        "PHOTO_URL8" => $fltr_arr->first()->DTIMG8,
                        "PHOTO_URL9" => $fltr_arr->first()->DTIMG9,
                        "PHOTO_URL10" => $fltr_arr->first()->DTIMG10,

                        "BANNER_URL1" => $fltr_arr->first()->DTBNR1,
                        "BANNER_URL2" => $fltr_arr->first()->DTBNR2,
                        "BANNER_URL3" => $fltr_arr->first()->DTBNR3,
                        "BANNER_URL4" => $fltr_arr->first()->DTBNR4,
                        "BANNER_URL5" => $fltr_arr->first()->DTBNR5,
                        "BANNER_URL6" => $fltr_arr->first()->DTBNR6,
                        "BANNER_URL7" => $fltr_arr->first()->DTBNR7,
                        "BANNER_URL8" => $fltr_arr->first()->DTBNR8,
                        "BANNER_URL9" => $fltr_arr->first()->DTBNR9,
                        "BANNER_URL10" => $fltr_arr->first()->DTBNR10,
                        "TOT_SURGERY" => count($T_DTL),
                        "FACILITY_DETAILS" => $T_DTL
                    ];
                }
                ...
                $data2 = DB::table('facility_section')
                    ->where('facility_section.DASH_SECTION_ID', 'AM')
                    ->get();
                $q["Questions"] = $data2->map(function ($item) {
                    return [
                        "QA1" => $item->DSQA1,
                        "QA2" => $item->DSQA2,
                        "QA3" => $item->DSQA3,
    ...
    function hosp_lst(Request $request)
    {
        if ($request->method() === 'POST') {
            ...
            if (isset($input['DASH_ID'], $input['DASH_SECTION_ID'], $input['LATITUDE'], $input['LONGITUDE'])) {
                $f_id = $input['DASH_ID'];
                $latt = $input['LATITUDE'];
                $lont = $input['LONGITUDE'];
                $S_id = $input['DASH_SECTION_ID'];
                ...
                try {
                    $data1 = DB::table('hospital_facilities_details')
                        ->join('facility', 'hospital_facilities_details.DASH_ID', '=', 'facility.DASH_ID')
                        ->join('pharmacy', 'hospital_facilities_details.PHARMA_ID', '=', 'pharmacy.PHARMA_ID')
                        // ->join('promo_banner', 'pharmacy.PHARMA_ID', '=', 'promo_banner.PHARMA_ID')
                        ->select(
                            'hospital_facilities_details.*',
                            ...
                            'pharmacy.LONGITUDE',
                            'pharmacy.PH_RATING',
                            // 'facility.DASH_SECTION_ID',
                            'facility.DN_DESCRIPTION as DESCRIPTION',
                    ...
                    * SIN(RADIANS('$latt'))))),2) as KM"),
                        ...
                        ->where(['hospital_facilities_details.DASH_ID'=> $f_id,'hospital_facilities_details.DASH_SECTION_ID'=> $S_id,'pharmacy.STATUS'=>'Active'])
                        ->orderby('KM')
                        ->get();
    ...
    function diaghosp_lst(Request $request)
    {
        if ($request->method() === 'POST') {
            $input = $request->json()->all();

            if (isset($input['DASH_ID'], $input['DASH_SECTION_ID'], $input['LATITUDE'], $input['LONGITUDE'], $input['PHARMA_ID'])) {
                $f_id = $input['DASH_ID'];
                $latt = $input['LATITUDE'];
                $lont = $input['LONGITUDE'];
                ...
                try {
                    ...
                    * SIN(RADIANS('$latt'))))),2) as KM"),
                        ...
                        ->where(['hospital_facilities_details.DASH_SECTION_ID'=> $S_id,'hospital_facilities_details.DASH_ID'=> $f_id,'hospital_facilities_details.PHARMA_ID'=> $phid,'pharmacy.STATUS'=>'Active'])
                        ->get();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PATIENT_ID']) && isset($input['PHARMA_ID']) && isset($input['BOOKING_ID'])) {
                ...
                $data1 = DB::table('booktest')
                    ...
                    ->where(['booktest.BOOKING_ID' => $BID, 'booktest.PHARMA_ID' => $PHID, 'booktest.PATIENT_ID' => $PID,'pharmacy.STATUS'=>'Active'])
    ...
    function vustst(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['TEST_ID'])) {
                ...
                $data1 = DB::table('clinic_testdata')
                    ->join('pharmacy', 'clinic_testdata.PHARMA_ID', '=', 'pharmacy.PHARMA_ID')
                    ->join('test_schedule', function ($join) {
                        $join->on('pharmacy.PHARMA_ID', '=', 'test_schedule.PHARMA_ID')
                            ->on('clinic_testdata.DEPT_ID', '=', 'test_schedule.DEPT_ID');
                    })
                    ->distinct('pharmacy.PHARMA_ID')
                    ->select(
                        'clinic_testdata.*',
        ...
        * SIN(RADIANS('$latt'))))),2) as KM")
                    ...
                    ->where('clinic_testdata.TEST_ID', '=', $TID)
                    ->where(['clinic_testdata.STATUS'=> 'Active','pharmacy.STATUS'=>'Active'])
                    ->where(function ($query) {
                        $query->where('clinic_testdata.DEPT_ID', '=', 'D1')
                            ->whereColumn('test_schedule.SUB_DEPT_ID', 'clinic_testdata.SUB_DEPT_ID')
                            ->orWhere(function ($query) {
                                $query->where('clinic_testdata.DEPT_ID', '=', 'D2')
                                    ->whereColumn('test_schedule.DEPT_ID', 'clinic_testdata.DEPT_ID');
                            });
                    })
                    ->distinct()
                    ->get();
                ...
                $groupedData = [];
                $lastRow = null; // Define a variable to store the last row

                foreach ($data1 as $row) {
                    $lastRow = $row; // Update the lastRow variable
                    if (!isset($groupedData[$row->PHARMA_ID])) {
                        $groupedData[$row->PHARMA_ID] = [
                            "PHARMA_ID" => $row->PHARMA_ID,
                            "PHARMA_NAME" => $row->ITEM_NAME,
                ...
                if ($lastRow) {
                    // $bnr["Catg_Banner"] = DB::table('master_testdata')
                    //     ->select('SUB_DEPT_ID AS BANNER_ID', 'SUB_DEPARTMENT AS BANNER_NAME', 'TEST_DESC AS DESCRIPTION', 'BANNER_URL')
                    //     ->where(['DASH_ID' => 'TS', 'SUB_DEPT_ID' => $lastRow->SUB_DEPT_ID])->take(1)->get();

                    $data1 = DB::table('dashboard_item')
                ->where('SUB_DEPT_ID',$lastRow->SUB_DEPT_ID)
                ->get();

                    $bnr["Catg_Banner"]= $data1->map(function ($item) {
                        return [
                            "BANNER_ID" => $item->DASH_ID,
                            // "DIS_ID" => $item->DIS_ID,
                            "BANNER_NAME" => $item->DASH_NAME,
    ...
    function itemorgantest(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $req->json()->all();
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['SUB_DEPT_ID']) && isset($input['ORGAN_ID'])) {

                $latt = $input['LATITUDE'];
                $lont = $input['LONGITUDE'];
                $test_catg = $input['SUB_DEPT_ID'];
                ...
                $data1 = DB::table('pharmacy')
                    ...
                    ->where(['clinic_testdata.SUB_DEPT_ID' => $test_catg, 'clinic_testdata.ORGAN_ID' => $org_id,'pharmacy.STATUS'=>'Active'])
                    ->orderby('clinic_testdata.TEST_NAME')
                    ->get();
                ...
                $data1 = DB::table('dashboard_item')
                ->where('SUB_DEPT_ID',$test_catg)
                ->get();

            $bnr1 = $data1->map(function ($item) {
                return [
                    "BANNER_ID" => $item->DASH_ID,
                    // "DIS_ID" => $item->DIS_ID,
                    "BANNER_NAME" => $item->DASH_NAME,
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['SUB_DEPT_ID']) && isset($input['ORGAN_ID'])) {
                ...
                ->select(                   
                    ...
                    ->where(['clinic_testdata.SUB_DEPT_ID' => $test_catg, 'clinic_testdata.ORGAN_ID' => $org_id, 'clinic_testdata.PHARMA_ID' => $pid])
                    ->orderby('TEST_NAME')
                    ->get();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                foreach ($data1 as $item) {
                    ...
                    $index = array_search($dashID, array_column($F1_DTL, 'DASH_ID'));
                ...
                $F_DTL["Scan_Test"] = array_values($F1_DTL);
    ...
    function avail_psch(Request $request)
    {
        if ($request->isMethod('post')) {
            date_default_timezone_set('Asia/Kolkata');
            $input = $request->json()->all();
            if (isset($input['PHARMAID'])) {
                ...
                $data = $this->getPSchDt($input['PHARMAID']);
    ...
    private function getPSchDt($P_ID)
    {
        $schDT = [];
        $cym = date('Ymd');
        $TestAvail = DB::table('test_schedule')->where(['PHARMA_ID' => $P_ID])->get();
            ...
            while ($startDate->lte($endDate) && $counter < 15) {
                ...
                if ($startDate->format('l') === $row->SCH_DAY) {
                    if (in_array($startDate->weekOfMonth, explode(',', $row->WEEK))) {
                        ...
                        $slots = $this->getPDtSlot($P_ID, $row->ID, $dates);
                        $data[] = [
                            "ID" => $row->ID,
                            "SCH_DT" => $dates,
                            "SCH_DAY" => $row->SCH_DAY,
                            ...
                            "BOOK_START_TIME" => $row->BOOK_START_TIME,
        ...
        if ($firstAvailable) {
            $firstAvailableIndex = $collection->search($firstAvailable);
            $schDT = array_slice($data, $firstAvailableIndex, 6);
        }
        return $schDT;
    }

    private function getPDtSlot($fid, $apid, $apdt)
    {
        $cdt = date('Ymd');
