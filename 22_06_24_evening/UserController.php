...
class UserController extends Controller
{
        ...
        if ($request->isMethod('post')) {
            ...
            if (isset($input['DRID']) && isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                foreach ($clinics as $clinic) {
                    if (isset($clinic['PHARMA_ID'])) {
                        $data[$clinic['PHARMA_ID']] = $clinic;
                    }
                }
                // $data = $data->merge($this->getClinicDrDt($latt, $lont, $drid));
                if (empty($data)) {
                    $response = ['Success' => false, 'Message' => 'Record not found', 'code' => 200];
                } else {
                    $response = ['Success' => true, 'data' => array_values($data), 'code' => 200];
                }
            } else {
                $response = ['Success' => false, 'Message' => 'Invalid input parameter', 'code' => 422];
            }
    ...
    function vuallsurg(Request $req)
    {
        ...
        //         $sur = DB::table('surgery')
        //             // ->join('hospital_facilities', 'surgery.SURG_NAME', '=', 'hospital_facilities.FACILITY_NAME')
        //             // ->distinct('SURGERY_TYPE')
        //             ->select(
        //                 // 'hospital_facilities.ID',
        //                 'surgery.SURG_ID AS SURGERY_ID',
        //                 'surgery.SURG_NAME AS SURGERY',
        //                 'surgery.DIS_ID',
        //                 'surgery.DIS_CATEGORY AS DIS_CATG',
        //                 'surgery.PHOTO_URL AS SURGERY_PHOTO',
        //                 'surgery.DASH_URL AS SURGERY_H_PHOTO',
        //                 'surgery.SURG_TYPE',
        //                 'surgery.SURG_SL AS POSITION',
        //                 'surgery.TYPE_SL AS GR_POSITION'
        //             )
        //             // ->distinct('ITEM_NAME')
        //             ->where(['surgery.TYPE_STATUS' => 'Active', 'surgery.STATUS' => 'Active'])
        //             ->orderby('surgery.TYPE_SL')
        //             ->orderby('surgery.SURG_SL')
        //             ->get();
        //         $data = array();
        //         $i = 1;
        //         $collection = collect($sur);
        //         $distinctValues = $collection->pluck('SURG_TYPE')->unique();
        //         foreach ($distinctValues as $row) {
        //             $fltr_arr = $sur->filter(function ($item) use ($row) {
        //                 return $item->SURG_TYPE === $row;
        //             });
        //             $Surg_DTL = $fltr_arr->values()->all();
        //             $tarr["surgery_" . $i . "_details"] = $Surg_DTL;
        //             $data = $data + $tarr;
        //             $tarr = [];
        //             $i++;
        //         }
        ...
        //         * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
        //          * SIN(RADIANS('$latt'))))),0) as KM"),
        //                 DB::raw('COUNT(drprofile.DIS_ID) as TOT_DR'),
        //             )
        //             ->groupby(
        //                 'pharmacy.PHARMA_ID',
        //                 'pharmacy.ITEM_NAME',
        ...
        //                 'pharmacy.Latitude'
        //             )
        //             ->where(['pharmacy.STATUS' => 'Active', 'pharmacy.CLINIC_TYPE' => 'Hospital'])
        //             ->orderbyraw('KM')
        //             ->take(25)
        //             ->get();
        //         $arr_cl["Hospital"] = $pharma;
    ...
    END) AS UNSIGNED) as AVAIL_DR")
                ...
                $SR_DTL = $fltr_dash->map(function ($item) {
                    return [
                        "DASH_ID" => $item->DASH_ID,
                        "DIS_ID" => $item->DIS_ID,
                        "SYM_ID" => $item->SYM_ID,
