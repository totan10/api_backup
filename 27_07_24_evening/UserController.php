            ...
            try {
                foreach ($BT as $row) {
                    foreach ($row['DETAILS'] as $detail) {
                        ...
                        // Calculate the requested booking count from $row['DETAILS']
                        $requestedBookingCount = count(array_filter($row['DETAILS'], function ($d) use ($detail) {
                            return $d['SCH_ID'] === $detail['SCH_ID'] &&
                                $d['SLOT_DATE'] === $detail['SLOT_DATE'] &&
                                $d['BOOK_DATE'] === $detail['BOOK_DATE'];
                        }));
