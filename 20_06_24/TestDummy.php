...
GROUP BY TEST_ID, clinic_testdata.PHARMA_ID, clinic_testdata.DEPARTMENT, HOME_COLLECT) as clinic_testdata"), function ($join) {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                foreach ($data1 as $row) {
                    ...
                    $groupedData[$row->DASH_TYPE]['FACILITY_DETAILS'][] = [
                        ...
                        "PHOTO_URL" => $row->URL_24X7_HI,
                        "BANNER_URL" => $row->URL_24X7_HB,
                    ];
                }
    ...
    //                 $data1 = DB::table('dashboard')
    //                 ->where('DASH_SECTION_ID', $f_id)
    //                 ->where('STATUS', 'Active')
    //                 ->orderby('DASH_ID')
    //                 ->get();

    //             // Create an associative array to store distinct departments by DASH_TYPE
    //             $departments = [];
    ...
    // function hn2nd_opinion(Request $request)
    // {
    //     if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    //         // $headers = apache_request_headers();
    //         // session_start();
    ...
    //                         "DASH_TYPE" => $row->DASH_TYPE,
    //                         "DESCRIPTION" => $row->GR_DESC,
    //                         "PHOTO_URL" =>  $row->URL_2NDGN_HG,
    //                         "BANNER_URL" => $row->URL_2NDGN_HGB,
    //                         "QA1" => $row->QA1,
    //                         "QA2" => $row->QA2,
    //                         "QA3" => $row->QA3,
    //                         "QA4" => $row->QA4,
