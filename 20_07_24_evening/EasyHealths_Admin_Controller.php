        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            foreach ($td as $row) {
                $fields = [
                    ...
                    'SAMPLE_ID' => $row['SPECIMEN_ID']??null,
    ...
    function testsrch(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                $notadded = DB::table('test_department')
                ->join('test_sub_dept','test_sub_dept.DEPT_ID','test_department.TEST_DEPT_ID')
                ->join('master_test', 'master_test.SUB_DEPT_ID', 'test_sub_dept.SUB_DEPT_ID')
                ->leftJoin('test_scanorgan', 'master_test.ORGAN_ID', 'test_scanorgan.ORGAN_ID')
                    ->select('master_test.TEST_ID',
                    'master_test.TEST_SL',
                        'master_test.TEST_NAME',
                        'master_test.TEST_TYPE',
                        'master_test.TEST_CODE',
                        // 'TEST_SAMPLE',
                        'test_sub_dept.SUB_DEPT_NAME as TEST_CATG',
                        'master_test.SUB_DEPT_ID',
                        'master_test.SPECIMEN_ID',
                        ...
                        'test_department.TEST_DEPT_ID as DEPT_ID',
                        'test_department.TEST_DEPT_NAME as DEPARTMENT',
    ...
    function pkg_dtls(Request $req)
{
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($input['ITEM_TYPE']) && isset($input['PHARMA_ID'])) {
            ...
            $data1 = DB::table('dashboard_section')
                ...
                ->leftJoin('package', function ($join) use ($p_id) {
                    $join->on('dashboard_item.DASH_ID', '=', 'package.LAB_PKG_ID')
                        ->where('package.PHARMA_ID', '=', $p_id)
                        ->orderby('package.PKG_ID');
                })
                ->leftJoin('package_details', function ($join) use ($p_id) {
                ...
                ->orderby('dashboard_item.DASH_ID')
                ->get();
    ...
    //             $lastChkOutTime = null;
    //             foreach (array_reverse($chkOutTimes) as $chkout) {
    //                 if ($chkout !== null) {
    //                     $lastChkOutTime = $chkout;
    //                     break;
    //                 }
    //             }
    ...
    //                                 return $item === null ? 0 : (int) $item;
    //                             })->toArray();
    //                             $availableAppointments = $data1->SLOT_APPNT - $totalAppointments;
    //                         } else {
    //                             $availableAppointments = $data1->SLOT_APPNT;
    //                             $totalAppointments = 0;
    //                             $BookedSl = [];
    //                         }
    ...
    //                             $bookingSerials = [];
    //                         } else {
    //                             $bookingSerials = range($cumulativeAppntCount, $cumulativeAppntCount + $data1->SLOT_APPNT - 1);
    //                             $cumulativeAppntCount += $data1->SLOT_APPNT;
    //                         }
    ...
    //                         $chkinTime->addHour();
    //                     }
    //                     // Log::info('Booked Serial: ' . $matchingSlots);
    //                     // return $slotString;
    //                 } else if ($data1->SLOT == '2') {
    //                     while ($chkinTime->lessThan($chkoutTime)) {
    //                         $endSlot = $chkinTime->copy()->addMinutes($intvl);
...
//                 $chkin = $row->CHK_IN_TIME;
    ...
    //                 // Convert all checkout times to Carbon instances
//                 $checkoutTimes = [
//                     $row->CHK_OUT_TIME ? Carbon::createFromFormat('h:i A', $row->CHK_OUT_TIME) : null,
//                     $row->CHK_OUT_TIME1 ? Carbon::createFromFormat('h:i A', $row->CHK_OUT_TIME1) : null,
//                     $row->CHK_OUT_TIME2 ? Carbon::createFromFormat('h:i A', $row->CHK_OUT_TIME2) : null,
//                     $row->CHK_OUT_TIME3 ? Carbon::createFromFormat('h:i A', $row->CHK_OUT_TIME3) : null,
//                 ];

    //                 // Filter out null values
//                 $checkoutTimes = array_filter($checkoutTimes);

    //                 // Find the latest checkout time
//                 $latestCheckoutTime = null;
//                 if (!empty($checkoutTimes)) {
//                     $latestCheckoutTime = max($checkoutTimes);
//                 }

    //                 // If no valid checkout time found, skip this iteration
...
// }
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['USERID']) && isset($input['PASSWORD'])) {
                $user = DB::table('pharmacy')
                    ->select('PHARMA_ID', 'USERID', 'ITEM_NAME', 'CLINIC_MOBILE', 'PASSWORD', 'ADDRESS', 'CITY', 'DIST', 'PIN', 'STATE', 'PHOTO_URL', 'LOGO_URL', 'CLINIC_TYPE', 'LATITUDE', 'LONGITUDE', 'CHEMBER_CT')
                    ->where(['USERID' => $input['USERID'], 'STATUS' => 'Active'])->first();
                if ($user != null) {
                    $data = [
    ...
    function vu_radiology(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                $data1 = DB::table('master_testdata')
                    ->join('clinic_testdata', 'master_testdata.TEST_ID', '=', 'clinic_testdata.TEST_ID')
                    ->select(
                        'master_testdata.TEST_ID',
                        'master_testdata.TEST_NAME',
                        'master_testdata.DEPT_ID',
                        'master_testdata.DEPARTMENT',
                        'master_testdata.SUB_DEPT_ID',
                        'master_testdata.TEST_CATG',
                        'clinic_testdata.PHARMA_ID',
                        'clinic_testdata.TEST_UC',
                        'clinic_testdata.TEST_SL',
                        'clinic_testdata.COST',
                        'clinic_testdata.SCH_DAY',
                        'clinic_testdata.WEEK',
                        'clinic_testdata.SCH_START_DT',
                        'clinic_testdata.SCH_FROM1',
                        'clinic_testdata.SCH_TO1',
                        'clinic_testdata.MAX_BOOK1',
                        'clinic_testdata.SCH_FROM2',
                        'clinic_testdata.SCH_TO2',
                        'clinic_testdata.MAX_BOOK2',
                        'clinic_testdata.SCH_FROM3',
                        'clinic_testdata.SCH_TO3',
                        'clinic_testdata.MAX_BOOK3',
                        'clinic_testdata.SCH_FROM4',
                        'clinic_testdata.SCH_TO4',
                        'clinic_testdata.MAX_BOOK4',
                        'clinic_testdata.DURATION',

                    )
                    ->where(['clinic_testdata.PHARMA_ID' => $PID, 'master_testdata.DEPARTMENT' => 'RADIOLOGY'])
                ...
                foreach ($data1 as $row2) {
                    if (!isset($groupedData[$row2->TEST_CATG])) {
                        $groupedData[$row2->TEST_CATG] = [
                            ...
                            "TEST_CATG" => $row2->TEST_CATG,
                            "SCH_DAY" => $row2->SCH_DAY,
                            "WEEK" => $row2->WEEK,
                            "SCH_START_DT" => $row2->SCH_START_DT,
                            "SCH_FROM1" => $row2->SCH_FROM1,
                            "SCH_TO1" => $row2->SCH_TO1,
                            "MAX_BOOK1" => $row2->MAX_BOOK1,
                            "SCH_FROM2" => $row2->SCH_FROM2,
                            "SCH_TO2" => $row2->SCH_TO2,
                            "MAX_BOOK2" => $row2->MAX_BOOK2,
                            "SCH_FROM3" => $row2->SCH_FROM3,
                            "SCH_TO3" => $row2->SCH_TO3,
                            "MAX_BOOK3" => $row2->MAX_BOOK3,
                            "SCH_FROM4" => $row2->SCH_FROM4,
                            "SCH_TO4" => $row2->SCH_TO4,
                            "MAX_BOOK4" => $row2->MAX_BOOK4,
        ...
        if ($req->isMethod('post')) {
            ...
            foreach ($td as $row) {
                if ($row['SUB_DEPT_ID'] == null) {
                    $schId = strtoupper(substr(md5($row['PHARMA_ID'] . $row['SCH_DAY'] . $row['DEPT_ID']), 0, 15));
                } else {
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($row[$maxBookCol] === null && $row[$chkInTimeCol] && $row[$chkOutTimeCol]) {
                        $chkinTime = Carbon::createFromFormat('h:i A', $row[$chkInTimeCol]);
                        $chkoutTime = Carbon::createFromFormat('h:i A', $row[$chkOutTimeCol]);
                        $minutesDiff = $chkinTime->diffInMinutes($chkoutTime, false);
    ...
    function avail_testsch(Request $request)
    {
        if ($request->isMethod('post')) {
            date_default_timezone_set('Asia/Kolkata');
            $input = $request->json()->all();
            if (isset($input['PHARMA_ID']) && isset($input['SCH_STATUS'])) {
                $sub_dept_id = $input['SUB_DEPT_ID'] ?? null;
                $data = [];

                $data = $this->getTestSchDt($input['PHARMA_ID'], $input['SCH_STATUS'], $sub_dept_id);
    ...
    private function getTestSchDt($P_ID, $SchSTS, $sub_dept_id)
    {
        $schDT = [];
        $cym = date('Ymd');
        if ($SchSTS === 'clinic') {
            $TestAvail = DB::table('test_schedule')->where(['PHARMA_ID' => $P_ID, 'SUB_DEPT_ID' => $sub_dept_id, 'SCH_STATUS' => $SchSTS])->get();
        } else {
            $TestAvail = DB::table('test_schedule')->where(['PHARMA_ID' => $P_ID, 'SUB_DEPT_ID' => $sub_dept_id, 'SCH_STATUS' => $SchSTS])->get();
        }
        ...
        if ($req->isMethod('post')) {
            ...
            foreach ($td as $row) {
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($fields[$maxBookCol] === null && !empty($row[$chkInTimeCol]) && !empty($row[$chkOutTimeCol])) {
                        $chkinTime = Carbon::createFromFormat('h:i A', $row[$chkInTimeCol]);
                        $chkoutTime = Carbon::createFromFormat('h:i A', $row[$chkOutTimeCol]);
                        $minutesDiff = $chkinTime->diffInMinutes($chkoutTime, false);
