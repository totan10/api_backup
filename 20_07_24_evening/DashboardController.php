...
class DashboardController extends Controller
{
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                foreach ($cldata['Clinic_Data'] as $clinic) {
                    if ($clinic->CLINIC_TYPE == 'Hospital') {
                        ...
                        //     ->join('hospital_facilities_details', 'facility_section.DASH_SECTION_ID', '=', 'hospital_facilities_details.DASH_SECTION_ID')
                        //     ->select([
                        //         'facility_section.DASH_SECTION_ID', 
                        //         'facility_section.DASH_SECTION_NAME', 
                        //         'facility_section.DS_DESCRIPTION', 
                        //         'facility_section.DS_POSITION',
                        //         'facility_section.DSIMG1', 'facility_section.DSIMG2', 'facility_section.DSIMG3', 'facility_section.DSIMG4', 'facility_section.DSIMG5', 'facility_section.DSIMG6', 'facility_section.DSIMG7', 'facility_section.DSIMG8', 'facility_section.DSIMG9', 'facility_section.DSIMG10', 
                        //         'facility_section.DSBNR1', 'facility_section.DSBNR2', 'facility_section.DSBNR3', 'facility_section.DSBNR4', 'facility_section.DSBNR5', 'facility_section.DSBNR6', 'facility_section.DSBNR7', 'facility_section.DSBNR8', 'facility_section.DSBNR9', 'facility_section.DSBNR10', 
                        //         'facility_section.DSQA1', 'facility_section.DSQA2', 'facility_section.DSQA3', 'facility_section.DSQA4', 'facility_section.DSQA5', 'facility_section.DSQA6', 'facility_section.DSQA7', 'facility_section.DSQA8', 'facility_section.DSQA9'
                        //     ])
                        //     ->distinct()
                        //     ->where('facility_section.DASH_SECTION_ID', '!=', 'AL')
                        //     ->where('hospital_facilities_details.PHARMA_ID',$pid)
                        //     ->orderBy('facility_section.DS_POSITION')
                        //     ->get();

                        // $DASH_Z["Dashboard"] = $dash1->map(function ($item) {
                        //     return [
                        //         "DASH_SECTION_ID" => $item->DASH_SECTION_ID,
                        //         "DASH_SECTION_NAME" => $item->DASH_SECTION_NAME,
                        //         "DESCRIPTION" => $item->DS_DESCRIPTION,
 ...
 END) AS UNSIGNED) as AVAIL_DR")
                ...
                // $dash1 = DB::table('dashboard')->where('CATEGORY', 'like', '%' . 'D' . '%')->where('STATUS', 'Active')->get();

                ## SECTION Surgery
                // $fltr_dash = $dash1->filter(function ($item) {
                //     return strpos($item->DASH_ITEM_TAGGED, 'SR') !== false && $item->INDASH === 'true';
                // });
                ...
                $I_DTL = DB::table('facility_section')
                    ...
                    ->join('facility', 'facility_type.DASH_TYPE_ID', '=', 'facility.DASH_TYPE_ID')
                    ->where(['facility_section.DS_STATUS' => 'Active', 'facility_type.DT_STATUS' => 'Active', 'facility.DN_STATUS' => 'Active'])
                    ->where('facility_section.DASH_SECTION_ID', 'SR')
                    ->orderBy('facility_type.DT_POSITION')
                    // ->orderBy('facility.DN_POSITION')
                    ->get();
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $data1 = DB::table('dashboard_section')
                    ...
                    ->join('test_sub_dept', 'dashboard_item.DASH_NAME', '=', 'test_sub_dept.SUB_DEPT_NAME')
                    ->join('master_test', function ($join) {
                        $join->on('test_sub_dept.DEPT_ID', '=', 'master_test.DEPT_ID')
                            ->on('test_sub_dept.SUB_DEPT_ID', '=', 'master_test.SUB_DEPT_ID');
                    })
                    ->join('test_scanorgan', 'master_test.ORGAN_ID', '=', 'test_scanorgan.ORGAN_ID')
                    ->select(
                    ...
                    ->where('test_sub_dept.DEPT_ID', '=', 'D1')
                ...
                $data1 = DB::table('dashboard_item')
                    ...
                    ->where(['dashboard_item.DASH_SECTION_ID' => 'S', 'dashboard_item.DASH_STATUS' => 'Active'])
                    ->orderby('dashboard_item.DASH_POSITION')
                    ->get();
    ...
    function labdashboard(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $F_DTL = $fltr_dash->map(function ($item) {
                    return [
                        "DASH_ID" => $item->DASH_ID,
                        "DIS_ID" => $item->DIS_ID,
                        "DASH_SECTION_ID" => $item->DASH_SECTION_ID,
