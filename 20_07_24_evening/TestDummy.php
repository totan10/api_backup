        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LONGITUDE']) && isset($input['LATITUDE'])) {
                ...
                $TST_DTL = DB::table('master_test')
                    ->join(DB::raw("(SELECT DISTINCT clinic_testdata.TEST_ID,clinic_testdata.REPORT_TIME,clinic_testdata.PRESCRIPTION,clinic_testdata.DEPARTMENT,clinic_testdata.HOME_COLLECT,clinic_testdata.TEST_CATG, MIN(clinic_testdata.COST) as MIN_COST FROM clinic_testdata
            WHERE clinic_testdata.DEPARTMENT='PATHOLOGY'
            GROUP BY TEST_ID,clinic_testdata.DEPARTMENT,clinic_testdata.HOME_COLLECT,clinic_testdata.TEST_CATG,clinic_testdata.REPORT_TIME,clinic_testdata.PRESCRIPTION) as clinic_testdata"), function ($join) {
                        $join->on('master_test.TEST_ID', '=', 'clinic_testdata.TEST_ID');
                    })
                    ->select('master_test.*', 'clinic_testdata.MIN_COST','clinic_testdata.TEST_CATG', 'clinic_testdata.HOME_COLLECT AS HOME_COLLECT','clinic_testdata.REPORT_TIME','clinic_testdata.PRESCRIPTION','clinic_testdata.DEPARTMENT')
