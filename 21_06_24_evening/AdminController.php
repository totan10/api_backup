...
class AdminController extends Controller
{
    ...
    function addsch(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            foreach ($td as $row) {
                $chkinTime = Carbon::createFromFormat('h:i A', $row['CHK_IN_TIME']);
                $chkoutTime = Carbon::createFromFormat('h:i A', $row['CHK_OUT_TIME']);
                $brkfrm = Carbon::createFromFormat('h:i A', $row['BREAK_FROM']);
                $brkto = Carbon::createFromFormat('h:i A', $row['BREAK_TO']);
                $minutesDiff = $chkinTime->diffInMinutes($chkoutTime, false);
                $brkDiff = $brkfrm->diffInMinutes($brkto, false);
                $maxbook = (($minutesDiff - $brkDiff) / $row['SLOT_INTVL']) * $row['SLOT_APPNT'];
                ...
                try {
                    ...
                    DB::table('dr_availablity')->where(['DR_ID' => $row['DR_ID'], 'SCH_STATUS' => 'NA'])->delete();
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                $data1 = DB::table('drprofile')
                    ...
                    ->select(
                        ...
                        'dr_availablity.BREAK_FROM',
                        'dr_availablity.BREAK_TO',
    ...
    function drdelay(Request $req)
    {
        ...
        try {
            DB::table('dr_availablity')->where(['ID' => $input['APPNT_ID']])->update($fields_d);
            DB::table('appointment')->where(['APPNT_ID' => $input['APPNT_ID'], 'APPNT_DT' => $input['APPNT_DT']])->update($fields_p1);
            $todayDoctors = DB::table('pharmacy')
                ->join('dr_availablity', 'pharmacy.PHARMA_ID', '=', 'dr_availablity.PHARMA_ID')
                ->join('drprofile', 'dr_availablity.DR_ID', '=', 'drprofile.DR_ID')
                ->distinct('drprofile.DR_ID')
    ...
    function vuupdt_facilities(Request $req)
    {
        ...
        if ($req->isMethod('post')) {
            ...
            $data = DB::table('dashboard')
                ...
                ->where([
                    'dashboard.DASH_SECTION_ID' => $input['DASH_SECTION_ID'],
                    'dashboard.STATUS' => 'Active',
                    'dashboard.DASH_TYPE' => $input['DASH_TYPE']
                ])
                ->get();

            $data1 = DB::table('surgery')
                ->leftJoin('hospital_facilities_details', function ($join) use ($pharmaId) {
                ...
                ->where([
                    'surgery.DASH_SECTION_ID' => $input['DASH_SECTION_ID'],
                    'surgery.SURG_TYPE' => $input['DASH_TYPE']
                ])
                ->get();

                $mergedData = $data->merge($data1);
//added new filter on 21-06-24
            ...
            if ($mergedData->isEmpty()) {
                ...
                $response = ['Success' => true, 'data' => $mergedData, 'code' => 200];
            }
        } else {
            $response = ['Success' => false, 'Message' => 'Method Not Allowed', 'code' => 200];
        }
    ...
    function delsch(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $req->json()->all();
            try {
                DB::table('dr_availablity')->where(['ID' => $input['SCH_ID']])->delete();
    ...
    public function vu_facilities(Request $req)
    {
        if ($req->isMethod('post')) {
            ...
            $groupedData = $this->groupData($data1);
            ...
            $groupedData1 = $this->groupData($data2);
