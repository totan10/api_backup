...
class DashboardController extends Controller
{
     ...
     * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
      * SIN(RADIANS('$latt'))))),2) as KM")
                ...
                foreach ($cldata as $data) {
                    ...
                    // $fltr_symp1 = $symDtlCollection->filter(function ($item) {
                    //     $validIds = [111,115,113];
                    //     return in_array($item['SYM_ID'], $validIds);
                    // });
                    $groupedDataItem = [
                        'PHARMA_ID' => $data->PHARMA_ID,
    ...
    function diagdash1(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                $promo_bnr = $promo_bnr->filter(function ($item) use ($pid) {
                    return $item->PHARMA_ID === $pid;
                });
 ...
 END) AS UNSIGNED) as AVAIL_DR")
                ...
                $data1 = DB::table('dashboard_item')
                    ...
                    ->join('master_test', 'sym_organ_test.TEST_ID', '=', 'master_test.TEST_ID')
                    ...
                    ->select(
                        ...
                        // 'master_test.TEST_CATG',
                        // 'master_test.DISCOUNT',
                        // 'master_test.HOME_COLLECT',
                        'master_test.ORGAN_ID',
                        // 'master_test.ORGAN_NAME',
                        // 'master_test.ORGAN_URL',
