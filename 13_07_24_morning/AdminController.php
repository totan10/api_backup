...
class AdminController extends Controller
{
    ...
    function avail_testsch(Request $request)
    {
        if ($request->isMethod('post')) {
            ...
            if (isset($input['PHARMA_ID']) && isset($input['SCH_STATUS'])) {
                ...
                $data = $this->getTestSchDt($input['PHARMA_ID'], $input['SCH_STATUS']);
    ...
    private function getTestSchDt($P_ID, $SchSTS)
    {
        $schDT = [];
        $cym = date('Ymd');
        if ($SchSTS === 'clinic') {
            $TestAvail = DB::table('test_schedule')->where(['PHARMA_ID' => $P_ID, 'SCH_STATUS' => $SchSTS])->get();
        } else {
            $TestAvail = DB::table('test_schedule')->where(['PHARMA_ID' => $P_ID, 'SCH_STATUS' => $SchSTS])->get();
        }
        ...
        foreach ($TestAvail as $row) {
            ...
            while ($startDate->lte($endDate) && $counter < $row->SCH_START_DAY) {
                if ($startDate->format('l') === $row->SCH_DAY) {
                    if (in_array($startDate->weekOfMonth, explode(',', $row->WEEK))) {
                        $dates = $startDate->format('Ymd');
        ...
        if ($firstAvailable) {
            $firstAvailableIndex = $collection->search($firstAvailable);
            $schDT = array_slice($data, $firstAvailableIndex, $row->SCH_START_DAY);
        }
