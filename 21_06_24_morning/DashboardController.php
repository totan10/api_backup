...
class DashboardController extends Controller
{
    ...
    END) AS UNSIGNED) as AVAIL_DR")
                ...
                // Retrieve distinct DIS_IDs from hospital_facilities_details
                $disIds = DB::table('hospital_facilities_details')
                    ->where('PHARMA_ID', $pid)
                    ->where('DASH_SECTION_ID', 'SR')
                    ->distinct()
                    ->pluck('DIS_ID')
                    ->toArray();

                // Retrieve SURG_DTL from 'surgery' table with whereIn clause
                $SURG_DTL = DB::table('surgery')
                    ->select('SURG_ID', 'SURG_NAME', 'SURG_TYPE', 'DIS_ID', 'DIS_CATEGORY', 'PHOTO_URL', 'DASH_URL')
                    ->whereIn('DIS_ID', $disIds) // Adding whereIn clause with $disIds
                    ->where('STATUS', 'Active')
                    ->where('TYPE_STATUS', 'Active')
                    ->orderBy('SURG_SL')
                    ->take(10)
                    ->get()
                    ->toArray(); // Convert Collection to array of stdClass objects

                // Assuming $SGB is the last element of $SURG_DTL and it's an array
                $fltr_promo_bnr = $promo_bnr->filter(function ($item) {
                    return $item->DASH_SECTION_ID === 'SR';
                });
                // Assuming $SGB is the last element of $SURG_DTL and it's an array
                $SGB = $fltr_promo_bnr->map(function ($item) {
                    return [
                        "PROMO_ID" => $item->PROMO_ID,
                        "HEADER_NAME" => $item->HEADER_NAME,
                        "PHARMA_ID" => $item->PHARMA_ID,
                ...
                })->take(3)->toArray(); // Convert Collection to array

                // Append $SGB array to $SURG_DTL array
                $SURG_DTL[] = $SGB;

                // Now you can assign $SURG_DTL to $SURG["Surgery"]
                $SURG["Surgery"] = $SURG_DTL;
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $TST_DTL  = DB::table('master_testdata')
                    ...
                    ->take(100)->get()->toArray();
                $fltr_promo_bnr = $promo_bnr->filter(function ($item) {
                    return $item->DASH_SECTION_ID === 'STB';
                });
                $STB["Test_Banner"] = $fltr_promo_bnr->map(function ($item) {
                    return [
                        "PROMO_ID" => $item->PROMO_ID,
