        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                foreach ($todayDoctors as $key => $doctor) {
                    ...
                    for ($i = 0; $i < count($nonNullChkInTimes); $i++) {
                        ...
                        if ($currentTime->lessThanOrEqualTo($checkOutTime)) {
                            ...
                            // $nextIndex = $i + 1;
                            // if ($nextIndex == count($chk) && !in_array($doctor->{$CHKINSTATUS[$i]}, ['CANCELLED', 'LEAVE'])) {
                            $doctor->CHK_IN_TIME = $doctor->{$chk[$i]};
                            $doctor->CHK_OUT_TIME = $doctor->{$chkout[$i]};
                    ...
                    $currentFromSlot = null;
                    // foreach ($slots as $slotArray) {
                    //     foreach ($slotArray as $slot) {
                    //         $fromTime = Carbon::createFromFormat('h:i A', $slot['FROM']);
                    //         $toTime = Carbon::createFromFormat('h:i A', $slot['TO']);
                    //         if ($currentTime->between($fromTime, $toTime)&&$  $slot['SLOT_STATUS'] === 'Available') {
                    //             $currentFromSlot = $slot['FROM'];
                    //             break 2;
                    //         }
                    //     }
                    // }
                    ...
                    if ($matchingSlot) {
                        $currentFromSlot = $matchingSlot['FROM'];
                    } elseif ($nextAvailableSlot) {
                        $currentFromSlot = $nextAvailableSlot['FROM'];
                    } else {
                        $currentFromSlot = null;
                    }
                ...
                if (empty($result['Today_Doctors'])) {
                    ...
                    $filtered_ldr = $uniqueDoctors->sortBy(function ($availableDoctor) use ($statusOrder) {
                        return [
                            $statusOrder[$availableDoctor['DR_STATUS']] ?? 999,
                            date("H:i", strtotime($availableDoctor['FROM'])),
                            date("H:i", strtotime($availableDoctor['CHK_IN_TIME'])),
                            $availableDoctor['DR_NAME']
                        ];
                    })->values()->all();
    ...
    function diagallscan(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                //     })
                //     ->leftJoin('test_scanorgan', 'clinic_testdata.ORGAN_ID', '=', 'clinic_testdata.ORGAN_ID')
                //     ->select(
                ...
                $data1 = DB::table('dashboard_section')
                    ...
                    ->join('clinic_testdata', function ($join) use ($pid) {
                        $join->on('dashboard_item.SUB_DEPT_ID', '=', 'clinic_testdata.SUB_DEPT_ID')
                            ->where('clinic_testdata.DEPT_ID', '=', 'D1')
                            // ->where('clinic_testdata.SUB_DEPT_ID', '=', 'SD2')
                            ->where('dashboard_item.DASH_SECTION_ID', '=', 'F')
                            ->where('clinic_testdata.PHARMA_ID', '=', $pid);
                    })
                    ->join('test_scanorgan', 'clinic_testdata.ORGAN_ID', '=', 'test_scanorgan.ORGAN_ID')
                    ->select(
                        'dashboard_section.DASH_SECTION_NAME',
                        'dashboard_item.ID',
                        'dashboard_item.DASH_SECTION_ID',
                    ...
                    ->distinct()
                    ->orderBy('test_scanorgan.ORGAN_ID', 'ASC')
                    ->get();
                ...
                $F_BNR["Banner"] = DB::table('promo_banner')
                    ->select('PROMO_ID', 'HEADER_NAME', 'PHARMA_ID', 'MOBILE_NO', 'DIS_ID', 'SYM_ID', 'PKG_ID', 'PROMO_NAME', 'DESCRIPTION', 'PROMO_URL', 'PROMO_DT', 'PROMO_VALID')
                    ->where(['DASH_SECTION_ID' => 'TS'])->take(3)->get();
