...
class EasyHealths_Admin_Controller extends Controller
{
    ...
    //                     $query->where('hospital_facilities_details.DASH_SECTION_ID', 'SP')
    //                           ->orWhere('hospital_facilities_details.DASH_SECTION_ID', 'SR');
    //                 });
    //             }
    //         })
...
//             ->where('hospital_facilities_details.PHARMA_ID', $pharmaId)
//             // ->where('hospital_facilities_details.SUPER_SPLTY', 'true')
//             ->whereIn('hospital_facilities_details.DASH_SECTION_ID', ['SP', 'SR', 'TV'])
//             ->select(
//                     'facility_section.DASH_SECTION_ID',
//                     'facility_section.DASH_SECTION_NAME',
//                     'facility_section.DS_DESCRIPTION',
...
//                     'hospital_facilities_details.IMAGE2_URL',
//                     'hospital_facilities_details.IMAGE3_URL',
//                     DB::raw("CASE WHEN hospital_facilities_details.DASH_SECTION_ID IN ('SR', 'SP') THEN NULL ELSE hospital_facilities_details.REMARK END AS REMARK")
//                 );
    ...
    if ($req->isMethod('post')) {
        ...
        if ($dsid == 'TV' ) {
            $additionalQuery = DB::table('hospital_facilities_details')
                ...
                ->leftJoin('facility_section', 'facility_section.DASH_SECTION_ID', '=', 'facility_type.DASH_SECTION_ID')
                ->where('hospital_facilities_details.PHARMA_ID', $pharmaId)
                ->whereIn('hospital_facilities_details.DASH_SECTION_ID', ['SP', 'SR','TV','TU'])
                ->select(
                    'facility_section.DASH_SECTION_ID',
                    'facility_section.DASH_SECTION_NAME',
                    'facility_section.DS_DESCRIPTION',
        ...
        // Filter and handle unique DASH_ID where REMARK is true
        foreach ($groupedData as &$typeData) {
            $uniqueDashId = [];
            $filteredFacilities = [];

            foreach ($typeData['FACILITY'] as $facility) {
                if ($facility['SUPER_SPLTY'] === 'true' && !in_array($facility['DASH_ID'], $uniqueDashId)) {
                    $uniqueDashId[] = $facility['DASH_ID'];
                    $filteredFacilities[] = $facility;
                } elseif ($facility['SUPER_SPLTY'] !== 'true') {
                    $filteredFacilities[] = $facility;
                }
            }

            $typeData['FACILITY'] = $filteredFacilities;
        }
