    ...
    function allsymptom_pathology1_dummy(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LONGITUDE']) && isset($input['LATITUDE'])) {
                ...
                //     )
                //     ->where(['dashboard_item.DASH_SECTION_ID' => 'S', 'dashboard_item.DASH_STATUS' => 'Active', 'clinic_testdata.DEPARTMENT' => 'PATHOLOGY'])
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                $dashboardPackages = DB::table('dashboard_item')
                    ->join('package', 'dashboard_item.ID', '=', 'package.LAB_PKG_ID')
                    ->join('package_details as pd', 'package.PKG_ID', '=', 'pd.PKG_ID')
        ...
        if ($request->isMethod('post')) {
            ...
            if (isset($input['DIS_ID']) && isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                // try {
                    ...
                    $fxd_banner = DB::table('dis_catg')->select('DIS_ID AS BANNER_ID', 'DIS_ID', 'DIS_DESC AS BANNER_DESC', 'DISBNR1 AS BANNER_URL')->get();
...
// }
    ...
    function globalscrh_dummy(Request $req)
    {
        ...
        // Query dashboard with selective columns
        $data7 = DB::table('dashboard_item')
            ->join('dashboard_section', 'dashboard_section.DASH_SECTION_ID', 'dashboard_item.DASH_SECTION_ID')
            ->whereIn('dashboard_section.DASH_SECTION_ID', ['B', 'C', 'D', 'G', 'H'])
            ->where('DASH_STATUS', 'Active')
            ->orderBy('ITEM_TYPE')
            ->select([
                'dashboard_item.ID',
                'dashboard_item.DASH_NAME',
                ...
                'dashboard_item.ITEM_TYPE AS DASH_TYPE',
                'dashboard_item.DASH_DESC AS DASH_DESCRIPTION',
                // 'AGE_TYPE',
                // 'FASTING',
                // 'GENDER_TYPE',
        ...
        foreach ($data7 as $row3) {
            $data->push([
                ...
                'DETAILS' => [
                    ...
                    'DASH_SECTION_ID' => $row3->DASH_SECTION_ID,
                    'DASH_SECTION_NAME' => $row3->DASH_SECTION_NAME,
                    'SECTION_SL' => $row3->DASH_POSITION,
                    'STATUS' => $row3->DASH_STATUS,
        ...
        // // Query sym_organ_test with selective columns
        $data8 = DB::table('sym_organ_test')
            ->join('master_test', 'master_test.TEST_ID', '=', 'sym_organ_test.TEST_ID')
            // ->join('dashboard', 'dashboard.DASH_ID', '=', 'sym_organ_test.DASH_ID')
            ->select([
                ...
                // 'dashboard.PHOTO_URL',
                'master_test.TEST_ID',
                'master_test.TEST_TYPE AS DASH_TYPE',
                'master_test.TEST_SL',
                'master_test.TEST_NAME',
                'master_test.TEST_CODE',
                'master_test.SAMPLE_NAME AS TEST_SAMPLE',
            ...
            ->where([
                ['sym_organ_test.DASH_SECTION_ID', '=', 'S'],
                ['master_test.TSTATUS', '=', 'Active']
            ])
            // ->orderBy('dashboard.DASH_POSITION')
            ->get();
