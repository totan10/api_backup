         ...
         * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
          * SIN(RADIANS('$latt'))))),2) as KM")
                ...
                $I_DTL = DB::table('facility_section')
                    ...
                    ->get()
                    ->map(function ($row2) {
                        $ds_dtl = DB::table('facility_section')->where('DASH_SECTION_ID', 'AH')->first();
                        $DsID = $ds_dtl->DASH_SECTION_ID;
                        $Dsname = $ds_dtl->DASH_SECTION_NAME;
                        return [
                            "DASH_ID" => $row2->DASH_ID,
         ...
         * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
          * SIN(RADIANS('$latt'))))),2) as KM")
                ...
                $data1 = DB::table('dashboard_item')
                    // ->where('dashboard_item.DASH_SECTION_ID', '=', 'F')
                    ->whereIn('dashboard_item.ID', [209, 54, 53, 51, 50])
                    ->join('dashboard_section', 'dashboard_item.DASH_SECTION_ID', '=', 'dashboard_section.DASH_SECTION_ID')
                    ->leftJoin('master_test', function ($join) {
                        $join->on('dashboard_item.SUB_DEPT_ID', '=', 'master_test.SUB_DEPT_ID');
                    })
                    ->leftJoin('test_scanorgan', 'master_test.ORGAN_ID', '=', 'test_scanorgan.ORGAN_ID')
                    ->select(
                ...
                foreach ($data1 as $item) {
                    ...
                    if (!isset($F1_DTL[$dashID])) {
                        ...
                        // Add ORGANS array only if ID is not 209
                        if ($dashID != 209) {
                            $F1_DTL[$dashID]["ORGANS"] = [];
                             $F1_DTL[$dashID]["DEPT_ID"] = $item->TEST_DEPT_ID;
                            $F1_DTL[$dashID]["SUB_DEPT_ID"]= $item->SUB_DEPT_ID;
                        }
                    }

                    // Skip adding organs for ID=209
                    if ($dashID == 209) {
                        continue;
                    }
...
* SIN(RADIANS('$latt'))))),2) as KM")
                ...
                $AS["Client3"] = array_values($groupedData);


                $data = $A + $DASH_A + $SPLST + $SYM + $CLINIC + $E + $D + $M + $N + $O + $P + $Q + $SURG + $R + $DIAG + $TST + $B + $C + $F + $DASH_BNR + $HOSP + $I + $J + $K + $L + $AJ + $AH + $AD + $AV + $AS;
    ...
    private function getTodayDoctor($pharmaId)
    {
        ...
        $data1 = DB::table('pharmacy')
            ->join('dr_availablity', function ($join) use ($day1, $weekNumber, $cdy,$pharmaId) {
    ...
    function testfromhome(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $request->json()->all();
            if (isset($input['LONGITUDE']) && isset($input['LATITUDE'])) {
                ...
                $pharmaIds = DB::table('clinic_testdata')
                    ->where('HOME_COLLECT', 'TRUE')
                    ->distinct()
                    ->pluck('PHARMA_ID');
                ...
                $data = DB::table('pharmacy')
                    ...
                    ->where('pharmacy.STATUS', 'Active')
                    ->whereIn('pharmacy.PHARMA_ID', $pharmaIds)
                    ->orderby('KM')
                    ->get();
                $groupedData = [];
                foreach ($data as $row) {
                ...
                $p['Clinic'] = $groupedData;


                // SECTION-#### SINGLE TEST
                $TST_DTL = DB::table('master_test')
                    ->join(DB::raw("(SELECT DISTINCT clinic_testdata.TEST_ID,clinic_testdata.PRESCRIPTION,clinic_testdata.HOME_COLLECT,clinic_testdata.TEST_CATG, MIN(clinic_testdata.COST) as MIN_COST FROM clinic_testdata
            WHERE clinic_testdata.HOME_COLLECT='TRUE'
            GROUP BY TEST_ID,clinic_testdata.TEST_CATG,clinic_testdata.HOME_COLLECT,clinic_testdata.PRESCRIPTION) as clinic_testdata"), function ($join) {
                        $join->on('master_test.TEST_ID', '=', 'clinic_testdata.TEST_ID');
                    })
                    ->join('test_sub_dept', 'master_test.SUB_DEPT_ID', '=', 'test_sub_dept.SUB_DEPT_ID')
                    ->select('master_test.*', 'clinic_testdata.MIN_COST', 'clinic_testdata.TEST_CATG', 'clinic_testdata.PRESCRIPTION', 'clinic_testdata.HOME_COLLECT', 'test_sub_dept.SDBNR1 as BANNER_URL')
    ...
    function pathologydata(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $request->json()->all();
            if (isset($input['LONGITUDE']) && isset($input['LATITUDE'])) {
                ...
                $data = DB::table('pharmacy')
                    ...
                    ->where(['clinic_testdata.DEPT_ID' => 'D2', 'pharmacy.STATUS' => 'Active'])
                    ->distinct()
                    ->get();
                $groupedData = [];
                foreach ($data as $row) {
    ...
    '), function ($join) {
                    ...
                    ->select(
                        ...
                        'clinic_testdata.QA6',
                        'clinic_testdata.MIN_COST',
                        'test_sub_dept.SUB_DEPT_ID',
                        'test_sub_dept.DEPT_ID',
                        'test_sub_dept.SDBNR1 as BANNER_URL'
                    )
                    ->where([
                        'dashboard_item.DASH_SECTION_ID' => 'S',
                        'dashboard_item.DASH_STATUS' => 'Active',
                        'test_sub_dept.DEPT_ID' => 'D2'
                    ])
                    ->orderBy('dashboard_item.DASH_POSITION')
                    ->get();
                ...
                // SECTION-#### SINGLE TEST
                $TST_DTL = DB::table('master_test')
                    ->join(DB::raw("(SELECT DISTINCT clinic_testdata.TEST_ID,clinic_testdata.PRESCRIPTION,clinic_testdata.TEST_CATG, MIN(clinic_testdata.COST) as MIN_COST FROM clinic_testdata
            WHERE clinic_testdata.DEPARTMENT='PATHOLOGY'
            GROUP BY TEST_ID,clinic_testdata.TEST_CATG,clinic_testdata.PRESCRIPTION) as clinic_testdata"), function ($join) {
                        $join->on('master_test.TEST_ID', '=', 'clinic_testdata.TEST_ID');
                    })
                    ->join('test_sub_dept', 'master_test.SUB_DEPT_ID', '=', 'test_sub_dept.SUB_DEPT_ID')
                    ->select('master_test.*', 'clinic_testdata.MIN_COST', 'clinic_testdata.TEST_CATG', 'clinic_testdata.PRESCRIPTION', 'test_sub_dept.SDBNR1 as BANNER_URL')
                    ->orderby('master_test.TEST_SL', 'asc')
                    ->take(100)->get()->toArray();
                ...
                $modifiedTest_bnr = DB::table('promo_banner')
                    ...
                    ->get();
                $modifiedResponse[] = $modifiedTest_bnr;
                $TST["Single_Test"] = $modifiedResponse;

                $samples = DB::table('test_sample')->get();

                $sampleDetails = [];

                foreach ($samples as $sample) {
                    $sampleTests = DB::table('master_test')
                        ->join(DB::raw("(SELECT DISTINCT clinic_testdata.TEST_ID,  MIN(clinic_testdata.COST) as MIN_COST FROM clinic_testdata
                    WHERE clinic_testdata.DEPARTMENT='PATHOLOGY'
                    GROUP BY TEST_ID) as clinic_testdata"), function ($join) {
                            $join->on('master_test.TEST_ID', '=', 'clinic_testdata.TEST_ID');
                        })
                        ->join('test_sub_dept', 'master_test.SUB_DEPT_ID', '=', 'test_sub_dept.SUB_DEPT_ID')
                        ->select('master_test.*', 'clinic_testdata.MIN_COST', 'test_sub_dept.SDBNR1 as BANNER_URL')
    ...
    function dephosp_lst(Request $request)
    {
            ...
            if (isset($input['DASH_ID'], $input['DASH_SECTION_ID'], $input['LATITUDE'], $input['LONGITUDE'])) {
                ...
                try {
                    ...
                    * SIN(RADIANS('$latt'))))),2) as KM"),
                        ...
                        ->where(['hospital_facilities_details.DASH_ID' => $f_id, 'hospital_facilities_details.DASH_SECTION_ID' => 'SP', 'pharmacy.STATUS' => 'Active'])
                        ->orderby('KM')
                        ->get();
