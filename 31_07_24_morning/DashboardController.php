        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $dash = DB::table('dashboard_section')
                    ...
                    ->orderby('dashboard_section.DSSL2')
                    ->orderby('dashboard_item.DISL2')
                    ->get();
                ...
                $dash1 = DB::table('facility_section')->orderBy('DSSL2')->get();
                ...
                //                 ->whereIn('dashboard_item.DASH_ID', [2, 5, 233, 237])
                //                 ->orderby('dashboard_section.DS_POSITION')
                //                 ->orderby('dashboard_item.DASH_POSITION')
                //                 ->get();
                ...
                //                 $fltr_dash = $dash->filter(function ($item) {
                //                     $validIds = [2, 5, 233, 237];
                //                     return in_array($item->DASH_ID, $validIds);
                //                 });
                //                 // $fltr_symp = $symDtlCollection->filter(function ($item) {
                //                 //     $validIds = [73, 75, 76];
                //                 //     return in_array($item['SYM_ID'], $validIds);
                //                 // });
                //                 $groupedDataItem = [
      ...
      * SIN(RADIANS('$latt'))))),2) as KM")
                ...
                foreach ($cldata as $data) {
                    $fltr_dash = DB::table('dashboard_section')
                        ...
                        // ->where('dashboard_section.DS_TAGGED', 'like', '%' . 'M' . '%')
                        ->where(['dashboard_section.DS_STATUS' => 'Active', 'dashboard_item.DASH_SECTION_ID' => 'A'])
                        ->whereIn('dashboard_item.DASH_ID', [2])
                        ->orderby('dashboard_section.DS_POSITION')
                        ->orderby('dashboard_item.DASH_POSITION')
                        ->get();
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                foreach ($cldata['Clinic_Data'] as $clinic) {
                    if ($clinic->CLINIC_TYPE == 'Hospital') {
                        ...
                        //Section - Service from Home
                        $SFH_DTL = DB::table('facility_section')
                            ->join('facility_type', 'facility_section.DASH_SECTION_ID', '=', 'facility_type.DASH_SECTION_ID')
                            ->join('facility', 'facility_type.DASH_TYPE_ID', '=', 'facility.DASH_TYPE_ID')
                            ->where(['facility_section.DS_STATUS' => 'Active', 'facility_type.DT_STATUS' => 'Active', 'facility.DN_STATUS' => 'Active'])
...
* SIN(RADIANS('$latt'))))),2) as KM")
                        ...
                        // $dash1 = DB::table('dashboard_section')
                        //     ->join('dashboard_item', 'dashboard_section.DASH_SECTION_ID', 'dashboard_item.DASH_SECTION_ID')
                        //     // ->leftjoin('dis_catg','dashboard_section.DASH_SECTION_ID','dashboard_item.DASH_SECTION_ID')
                        //     // ->where('dashboard_section.DS_TAGGED', 'like', '%' . 'M' . '%')
                        //     ->where(['dashboard_section.DS_STATUS' => 'Active', 'dashboard_item.DASH_SECTION_ID' => 'A'])
                        ...
                        $filtr_dash = $dash->filter(function ($item) {
                            return in_array($item->DASH_ID, [2, 5, 233, 237]);
                        });
                        ...
                        foreach ($cldata1 as $data) {
                            ...
                            $fltr_dash = $dash->filter(function ($item) {
                                return in_array($item->DASH_ID, [2]);
                            });
 ...
 END) AS UNSIGNED) as AVAIL_DR")
                ...
                //     ->select('TEST_ID', 'TEST_NAME', 'TEST_CATG', 'DISCOUNT', 'HOME_COLLECT', 'ORGAN_ID', 'ORGAN_NAME', 'ORGAN_URL', 'TEST_DESC', 'DEPARTMENT as CATEGORY', 'COST', 'KNOWN_AS', 'FASTING', 'GENDER_TYPE', 'AGE_TYPE', 'REPORT_TIME', 'PRESCRIPTION', 'ID_PROOF', 'test_sub_dept.SDBNR1 as BANNER_URL', 'QA1', 'QA2', 'QA3', 'QA4', 'QA5', 'QA6')
                //     ->where(['PHARMA_ID' => $pid])
                //     ->orderBy('TEST_SL')
                ...
                $data1 = DB::table('dashboard_item')
                    ->join('sym_organ_test', function ($join) {
                        $join->on('sym_organ_test.DASH_ID', '=', 'dashboard_item.DASH_ID')
                            ->where([
                ...
                $data1 = DB::table('dashboard_item')
                    ->join('sym_organ_test', function ($join) {
                        $join->on('sym_organ_test.DASH_ID', '=', 'dashboard_item.DASH_ID')
                            ->where([
                                'dashboard_item.DASH_SECTION_ID' => 'T',
                                'dashboard_item.DASH_STATUS' => 'Active'
                            ]);
                    })
                    ->join('clinic_testdata', function ($join) use ($pid) {
                ...
                $data10 = DB::table('dashboard_section')
...
* SIN(RADIANS('$latt'))))),2) as KM")
                        ...
                        foreach ($cldata1 as $data) {
                            ...
                            $groupedDataItem = [
                                ...
                                'LONGITUDE' => $data->LONGITUDE,
                                'KM' => $data->KM,
                                'DETAILS' => $dash->map(function ($item) {
                                    return [
                                        "DASH_ID" => $item->DASH_ID,
                                        "DASH_NAME" => $item->DASH_NAME,
                                        "DASH_SECTION_ID" => $item->FACILITY_ID,
 ...
 END) AS UNSIGNED) as AVAIL_DR")
                ...
                $G_DTL1 = $fltr_dash->map(function ($item) {
                    return [
                        "DASH_SECTION_ID" => $item->DASH_SECTION_ID,
                        "DASH_SECTION_NAME" => $item->DASH_SECTION_NAME,
                        // "DASH_TYPE" => $item->DASH_TYPE,
                        "DESCRIPTION" => $item->DS_DESCRIPTION,
                        "BANNER_URL1" => $item->DS_BNR1,
                        "BANNER_URL2" => $item->DS_BNR2,
                        "BANNER_URL3" => $item->DS_BNR3,
                        "BANNER_URL4" => $item->DS_BNR4,
                        "BANNER_URL5" => $item->DS_BNR5,
                        "BANNER_URL6" => $item->DS_BNR6,
                        "BANNER_URL7" => $item->DS_BNR7,
                        "BANNER_URL8" => $item->DS_BNR8,
                        "BANNER_URL9" => $item->DS_BNR9,
                        "BANNER_URL10" => $item->DS_BNR10,
    ...
    function labdashboard1(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $S_DTL = $symptomaticData->groupBy('DASH_ID')->map(function ($group) {
     ...
     * COS(RADIANS('$latt')) * COS(RADIANS(pharmacy.Longitude - '$lont')) + SIN(RADIANS(pharmacy.Latitude)) 
      * SIN(RADIANS('$latt'))))),2) as KM")
                ...
                $data1 = DB::table('dashboard_section')
                    ...
                    ->where('master_test.DEPT_ID', '=', 'D1')
                    ->whereIn('dashboard_item.DASH_ID', [54, 53, 51, 50])
                    ->where('dashboard_item.DASH_SECTION_ID', '=', 'F')
                    ->orderBy('dashboard_item.DASH_ID', 'desc')
                    ->get();
    ...
    function cltestdash(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                //SECTION-Z #### DASHBOARD
                $fltr_dash = $dash->filter(function ($item) {
                    return $item->DASH_SECTION_ID === 'A' && in_array($item->DASH_ID, [208, 209, 210, 211, 212, 213]);
                });
