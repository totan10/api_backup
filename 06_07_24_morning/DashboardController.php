    ...
    function hndashboard1(Request $request)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                // $surg = DB::table('facility')->where('DN_TAG_SECTION', 'like', '%' . 'SR' . '%')->orderby('DN_POSITION')->get();
    ...
    function dashboard(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                //SECTION-#### SURGERY
                // $surg=DB::table('facility')
                // ->where('DN_TAG_SECTION', 'like', '%' . 'SR' . '%')
                // ->orderby('DN_POSITION')
                // ->get();

                // // $fltr_dash = $surg->filter(function ($item) {
                // //     return $item->DASH_SECTION_ID === 'SR';
                // // });
                // $SURG_DTL = $surg->map(function ($item) {
                //     return [
