...
class AdminEasyHealths extends Controller
{
    ...
    function add_service(Request $req)
    {
        ...
        if ($req->hasFile('file')) {
            $suffix = substr(md5(now()->timestamp), 0, 10);
            $fileExtension = $req->file('file')->getClientOriginalExtension();
        ...
        $fields[$input['SL_TAG']] = $input['DN_POSITION'];
        if ($url !== null) {
    ...
    //         ];
    //         $fields[$input['SL_TAG']] = $input['DS_POSITION'];
    //         $fields1[$input['SL_TAG1']] = $input['DS_POSITION'];
    ...
    function add_facility_section(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $response = array();
            $input = $req->all();
            ...
            $fields = [
                ...
                'DS_DESCRIPTION' => $input['DS_DESCRIPTION'],
                'DS_STATUS' => $input['DS_STATUS'],
                'DS_TYPE' => $input['DS_TYPE'] ?? NULL,
            ];

            $fields[$input['SL_TAG']] = $input['DS_POSITION'] ?? NULL;
            ...
            DB::table('facility_section')->insert($fields);

            $response = ['Success' => true, 'Message' => 'Service added successfully.', 'PHOTO_URL' => $url, 'code' => 200];
        } else {
            $response = ['Success' => false, 'Message' => 'Method Not Allowed.', 'code' => 405];
        }
        return $response;
    ...
    function updt_facility_section1(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $response = array();
            $input = $req->all();
            ...
            $fields1 = [
                ...
                'DASH_DESC' => $input['DS_DESCRIPTION'],
                'DASH_STATUS' => $input['DS_STATUS'],
                'ITEM_TYPE' => $input['DS_TYPE'] ?? NULL,
            ];
            $fields[$input['SL_TAG']] = $input['DS_POSITION'] ?? NULL;
            $fields1[$input['SL_TAG1']] = $input['DS_POSITION'] ?? NULL;
            if (isset($input['DS_TAGGED'])) {
                $fields['DS_TAGGED'] = $input['DS_TAGGED'];
            }
            if ($url !== null) {
    ...
    function add_grservice(Request $req)
    {
        ...
        $fields[$input['SL_TAG']] = $input['DT_POSITION'];
        if ($url !== null) {
            $fields[$input['IMAGE_KEY']] = $url;
        }
    ...
    function updt_ditemfaq(Request $req)
    {
        if (!$req->isMethod('post')) {
            return ['Success' => false, 'Message' => 'Method Not Allowed.', 'code' => 405];
        }
    ...
    function updt_dssec1(Request $req)
    {
        if (!$req->isMethod('post')) {
            return ['Success' => false, 'Message' => 'Method Not Allowed.', 'code' => 405];
        }
    ...
    function add_dashitem(Request $req)
    {
        ...
        $fields[$input['SL_TAG']] = $input['DASH_POSITION'];
    ...
    function updt_dashitem(Request $req)
    {
        ...
        $url = null;
        if ($req->hasFile('file')) {
            $suffix = $suffix = strtoupper(substr(md5($input['ID'] . $input['IMAGE_KEY']), 0, 10));
            $fileExtension = $req->file('file')->getClientOriginalExtension();
            $fileName = $suffix . "." . $fileExtension;
        ...
        $fields1 = [
            // 'DASH_SECTION_ID' => $input['FACILITY_ID'] ?? null,
        ...
        $fields[$input['SL_TAG']] = $input['DASH_POSITION'];
        $fields1[$input['SL_TAG1']] = $input['DASH_POSITION'];
        if ($url !== null) {
            $fields[$input['IMAGE_KEY']] = $url;
            $fields1[$input['IMAGE_KEY1']] = $url;
        }
        ...
        try {
            ...
            if (str_contains($input['DASH_ITEM_TAGGED'], 'H')) {
                DB::table('facility_section')
    ...
    function updt_splst(Request $req)
    {
        ...
        $fields[$input['SL_TAG']] = $input['DIS_SL'];
        if ($url !== null) {
            $fields[$input['IMAGE_KEY']] = $url;
        }
    ...
    function updt_symptom(Request $req)
    {
        ...
        $fields[$input['SL_TAG']] = $input['SYM_SL'];
        if ($url !== null) {
            $fields[$input['IMAGE_KEY']] = $url;
        }
    ...
    function updt_test(Request $req)
    {
        ...
        $fields[$input['SL_TAG']] = $input['TEST_SL'];
