...
GROUP BY TEST_ID, clinic_testdata.PHARMA_ID, clinic_testdata.DEPARTMENT, HOME_COLLECT) as clinic_testdata"), function ($join) {
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['PHARMA_ID'])) {
                ...
                $dashboardPackages = DB::table('dashboard')
                    ...
                    ->whereIn('dashboard.DASH_SECTION_ID', ['B', 'C', 'D', 'G', 'H', 'S', 'T'])
    ...
    function globalscrh_dummy(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                // $data4 = DB::table('surgery')->get();
                // foreach ($data4 as $row5) {
                //     $sudtl = [];
                //     $sudtl['DETAILS'] = [
                //         "DIS_ID" => $row5->DIS_ID,
                ...
                * SIN(RADIANS('$latt'))))),2) as KM"), )
                ...
                $data7 = DB::table('dashboard')
                    ->whereIn('DASH_SECTION_ID', ['B', 'C', 'D', 'G', 'H', 'S', 'T', 'AG', 'AH', 'AI', 'AJ', 'AL', 'AP', 'SR'])
                    ->where('STATUS', 'Active')
                    ->orderby('DASH_TYPE')
                    ->get();
                ...
                foreach ($data7 as $row3) {
                    ...
                    // Conditional check to add '24x7 ' before DASH_NAME and DASH_TYPE if DASH_SECTION_ID is 'AG'
    ...
    function servicescrh_dummy(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE'])) {
                ...
                $data1 = DB::table('dashboard')
                    ->whereIn('DASH_SECTION_ID', ['AG', 'AH', 'AI', 'AJ', 'AL', 'AP', 'SR'])
                    ->where('STATUS', 'Active')
                    ->orderby('DASH_TYPE')
                    ->get();
    ...
    function opinion_depts_dummy(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $req->json()->all();
            if (isset($input['DR_ID'])) {
                ...
                $endDate = Carbon::today()->addDays(30)->format('Ymd');

                // Fetch appointments data
                $appointmentsData = DB::table('appointment')
                    ->select('APPNT_DT', 'PATIENT_ID', 'PHARMA_ID')
                    ->where('DR_ID', $doctorId)
                    ->whereBetween('APPNT_DT', [$startDate, $endDate])
                    ->get();

                // Group appointments by date and count patients and chambers
                $groupedAppointments = [];
                foreach ($appointmentsData as $appointment) {
                    $date = $appointment->APPNT_DT;
                    ...
                    $groupedAppointments[$date]['TOT_PATIENT']++;
                    $groupedAppointments[$date]['TOT_CHEMBERS']++;

                    // Use a set to count unique PHARMA_IDs per date
                    $groupedAppointments[$date]['TOT_CHEMBERS'] = count(array_unique(array_column($appointmentsData->where('APPNT_DT', $appointment->APPNT_DT)->toArray(), 'PHARMA_ID')));
                }
                $data = array_values($groupedAppointments);
                if ($data == null) {
                    $response = ['Success' => false, 'Message' => 'No Appointment Found', 'code' => 200];
                } else {
                    $response = ['Success' => true, 'data' => $data, 'code' => 200];
                }
