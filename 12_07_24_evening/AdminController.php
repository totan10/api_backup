...
class AdminController extends Controller
{
    ...
    function addsch_radiology(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $req->json()->all();
            $td = $input['ADD_SCH'];
            ...
            foreach ($td as $row) {
                $schId = strtoupper(substr(md5($row['TEST_CATG'] . $row['PHARMA_ID'] . $row['SCH_DAY']), 0, 15));
                $fields = [
                    'SCH_ID' => $schId,
                    'TEST_CATG' => $row['TEST_CATG'],
                ...
                for ($i = 1; $i <= 4; $i++) {
                    ...
                    if ($row[$maxBookCol] === null && $row[$chkInTimeCol] != null && $row[$chkOutTimeCol] != null) {
                        ...
                        $chkoutTime = Carbon::createFromFormat('h:i A', $row[$chkOutTimeCol]);
                        $minutesDiff = $chkinTime->diffInMinutes($chkoutTime, false);
                        $maxbook = ($minutesDiff / $row['DURATION']);
                        $fields[$maxBookCol] = $maxbook;
                    } elseif ($row[$chkOutTimeCol] === null && $row[$chkInTimeCol] != null && $row[$maxBookCol] != null) {
                        $chkinTime = Carbon::createFromFormat('h:i A', $row[$chkInTimeCol]);
                        $minutesDiff = $row[$maxBookCol] * $row['DURATION'];
                        $chkoutTime = $chkinTime->copy()->addMinutes($minutesDiff);
                        $fields[$chkOutTimeCol] = $chkoutTime->format('h:i A');
                    } else {
                        $fields[$maxBookCol] = $row[$maxBookCol];
                ...
                try {
                    $existingSchedule = DB::table('clinic_testdata')->where('SCH_ID', $schId)
                        ->where('SCH_STATUS', '!=', 'NA')
                        ->first();
                    if ($existingSchedule) {
                        $response = ['Success' => false, 'Message' => 'Schedule ID already exists.', 'code' => 200];
                        ...
                        DB::table('clinic_testdata')->insert($fields);
                        DB::table('clinic_testdata')->where(['TEST_CATG' =>  $row['TEST_CATG'], 'PHARMA_ID' =>  $row['PHARMA_ID'], 'SCH_STATUS' => 'NA'])->delete();
                        $response = ['Success' => true, 'Message' => 'Radiology Test schedule added successfully.', 'code' => 200];
                    }
                } catch (\Throwable $th) {
                    $response = ['Success' => false, 'Message' => $th->getMessage(), 'code' => 200];
                }
    ...
    function addsch_pathology(Request $req)
    {
        if ($req->isMethod('post')) {
            $input = $req->json()->all();

            $td = $input['ADD_PATH_SCH'];

            foreach ($td as $row) {
                $schId = strtoupper(substr(md5($row['PHARMA_ID'] . $row['SCH_DAY']), 0, 15));
                $fields = [
                    'SCH_ID' => $schId,
                    'DEPT_ID' => $row['DEPT_ID'],
                    'PHARMA_ID' => $row['PHARMA_ID'],
        ...
        if ($req->isMethod('post')) {
            ...
            if ($data) {
                ...
                foreach ($todaychmbers as $chm) {
                    if (!is_null($chm->CHEMBER_NO)) $bookedSerials[] = $chm->CHEMBER_NO;
                    if (!is_null($chm->CHEMBER_NO1)) $bookedSerials[] = $chm->CHEMBER_NO1;
                    if (!is_null($chm->CHEMBER_NO2)) $bookedSerials[] = $chm->CHEMBER_NO2;
                    if (!is_null($chm->CHEMBER_NO3)) $bookedSerials[] = $chm->CHEMBER_NO3;
                }
    ...
    // function vuupdt_facility(Request $req)
    // {
    //     // if (isset($headers['Authorization']) && $headers['Authorization'] === $_SESSION['TOKEN']) {           
    //     if ($req->isMethod('post')) {
    //         $input = $req->json()->all();
    ...
    //                     ->where(['hospital_facilities_details.PHARMA_ID' => $pharmaId, 'facility.DASH_ID' => $dsid]);
    //             })
    //             ->select(
    ...
    function vupath_sch(Request $req)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            $input = $req->json()->all();
            if (isset($input['PHARMA_ID']) && isset($input['DEPT_ID'])) {
                ...
                $data = DB::table('test_schedule')->where(['PHARMA_ID'=>$input['PHARMA_ID'],'DEPT_ID'=>$input['DEPT_ID']])->get();
