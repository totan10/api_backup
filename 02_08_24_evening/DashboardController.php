        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                $promo_bnr1 = DB::table('promo_banner')
                    // ->join('dashboard_item', 'dashboard_item.DASH_SECTION_ID', '=', 'dashboard_section.DASH_SECTION_ID')
                    ->leftJoin('dashboard_item', function ($join) {
                        $join->on('dashboard_item.ID', '=', 'promo_banner.DASH_ID')
                            ->where(['promo_banner.PHARMA_ID' => 0, 'promo_banner.PROMO_TYPE' => 'Slider']);
                    })
                    ->select(
                        'promo_banner.*',
                        'dashboard_item.DI_IMG1 as PHOTO_URL1',
                ...
                function calculateAvailableDoctors($pid, $did)
                {
                    ...
                    if (!empty($ldr['livedoctor'])) {
                        ...
                        $filtered_ldr = array_filter($ldr['livedoctor'], function ($doctor) {
                            return in_array($doctor['DR_STATUS'], ["IN", "TIMELY", "DELAY"]);
                        });
                        $ldr['livedoctor'] = array_values($filtered_ldr);
                    }
                    return count($ldr['livedoctor']);
                        ...
                        $SPLST_DTL = DB::table('dis_catg')
                            ...
                            ->map(function ($item) use ($pid) {
                                return [
                                    ...
                                    "BANNER_URL10" => $item->DISBNR10,
                                    "TOT_DR" => $item->TOT_DR,
                                    "AVAIL_DR" => calculateAvailableDoctors($item->DIS_ID, $pid),
                                    "Questions" => [
                                        "QA1" => $item->DISQA1,
                                        "QA2" => $item->DISQA2,
                                        "QA3" => $item->DISQA3,
    ...
    function diagdash1(Request $request)
    {
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if (isset($input['LATITUDE']) && isset($input['LONGITUDE']) && isset($input['PHARMA_ID'])) {
                ...
                $promo_bnr1 = DB::table('promo_banner')
                ->where(['promo_banner.PROMO_TYPE' => 'Slider'])
                    ->select(
                        'promo_banner.*',
                        // 'dashboard_item.DI_IMG1 as PHOTO_URL1',
                        // 'dashboard_item.DI_IMG2 as PHOTO_URL2',
